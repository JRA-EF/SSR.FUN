...
```json
{
  ...
  "evidence": [
    ...
    "Committed on main, pushed to origin/main (GitHub), then deployed to production via `vercel --prod` -- see the immediately following commit/deploy record for the exact hash and deployment ID (recorded post-commit per this project's no-amend policy)."
  ]
}
```
