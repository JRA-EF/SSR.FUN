...
```json
{
  "id": "DEC-0133",
  "date": "2026-08-21",
  "status": "confirmed-implemented-not-deployed",
  "decision": "Implemented a USDC-settled fee model for ssr_protocol -- both the mint-time fee and the Annualized TVL fee now crystallize into a shared per-Reserve fee vault (instead of minting straight to final Reserve-Token destinations), which a new, separate, permissionless settlement pipeline redeems for underlying assets, swaps to USDC via Jupiter (off-chain, keeper-signed -- the program never signs a swap), and distributes to the Protocol Treasury and the Reserve's Manager recipient(s). Modeled conceptually on Reserve Protocol's mint-dilute/redeem/trade/distribute pattern, per Creator's explicit direction (DEC-0132's follow-up). Implemented, validated, and committed in full this pass. Per Creator's explicit instruction, NOT deployed to Mainnet and NO live settlement transaction was executed -- this entry is also the requested pre-deployment report.",
  "context": "Following DEC-0132's investigation and deferral, Creator gave a precise, detailed specification: lazy continuous TVL-fee accrual (already substantially implemented, see below), crystallization by minting to a dedicated fee vault, USDC settlement via redeem-then-Jupiter-swap-then-distribute with explicit slippage/route validation, permissionless/keeper-compatible idempotent settlement kept separate from accrual, and safe retry for an asset with no valid Jupiter route. Explicit instructions: audit first, implement and validate locally, do NOT upgrade the Mainnet program or execute Mainnet settlement, and report proposed account/instruction changes, exact fee math, security implications, cost estimates, migration treatment, Mainnet compatibility, and the exact deployment plan requiring Creator approval. A design plan (accounts/instructions/fee math/security/costs/migration/compatibility/deployment-plan) was presented and approved via plan mode before any code was written.",
  "rationale": "AUDIT (before any code changed): confirmed the existing TvlAccrual/checkpoint_tvl_accrual mechanism (state/tvl_accrual.rs, instructions/accrue_fees.rs) ALREADY satisfies requirements 1-4 and most of 7 of the spec -- a genuine time-weighted (Uniswap-V2-style) cumulative accumulator, checkpointed for free on every mint/redeem/seed, settled lazily by a separate permissionless instruction, never per-second execution. This pass deliberately does not touch that accrual-RATE logic at all, only the crystallization DESTINATION.\n\nPROPOSED/IMPLEMENTED ACCOUNTS (all new, additive PDAs -- zero changes to Reserve/FeeConfig/TvlAccrual/ManagerFeeRecipients's existing layouts): `FeeSettlement` (per-Reserve, tracks the fee vault's current protocol:manager composition and the pending-settlement ratio), the fee vault itself (an ATA owned by `fee_vault_authority`), per-asset + USDC settlement staging ATAs (owned by `settlement_authority`), and `SettlementKeeperConfig` (a NEW, separate, protocol-wide singleton PDA holding the configured keeper wallet).\n\nCRITICAL SELF-CAUGHT DESIGN BUG: the first draft added `fee_settlement_keeper: Pubkey` directly onto the existing, already-initialized-on-Mainnet `ProtocolConfig` struct. This is unsafe and would have broken the ENTIRE live program on upgrade: every single instruction reads ProtocolConfig, so Anchor's deserialization of the new, larger struct shape against the still-smaller on-chain account data fails immediately -- and a `realloc` constraint can't rescue this, since realloc only runs AFTER a successful account load, an impossible bootstrap the moment the new field is unconditionally read. Caught and fixed BEFORE compiling, by moving the keeper into `SettlementKeeperConfig`, a wholly separate singleton PDA -- the same lesson this codebase already learned once for TvlAccrual/ManagerFeeRecipients at the per-Reserve level, now confirmed to also apply at the protocol-wide level. `ProtocolConfig`'s own header comment now documents this explicitly as a permanent rule.\n\nPROPOSED/IMPLEMENTED INSTRUCTIONS: `set_fee_settlement_keeper` (admin-gated, writes SettlementKeeperConfig); `mint_reserve_tokens_in_kind`/`accrue_fees` MODIFIED (crystallization destination only -- fee MATH itself, split_configured_bps/split_total_fee, is completely unchanged) to mint both shares into the shared fee vault instead of instant-mint-to-treasury (protocol) / pending-counter-then-claim (manager); `redeem_fee_vault_shares` (permissionless -- burns fee-vault shares, stages the proportional per-asset entitlement using the IDENTICAL math redeem_reserve_tokens_in_kind already uses for a normal Sell); `approve_settlement_swap` (the ONLY keeper-gated instruction in the whole pipeline -- grants a bounded, per-call SPL delegate approval, never a standing one, over exactly one asset's staged amount; the program never signs a swap, matching api/mainnet/jupiter-swap.ts's existing 'server never signs a Mainnet write' convention); `distribute_fee_usdc` (permissionless, idempotent -- always splits the REAL current USDC staging balance, never a remembered/expected total, by the current pending-settlement ratio, reusing the existing apportion_to_recipients for multi-recipient Manager distribution unchanged).\n\nFEE MATH (exact): crystallization reuses split_configured_bps/split_total_fee verbatim (unchanged). A new, generalized `split_by_weight(amount, protocolWeight, managerWeight)` (fee_math.rs + an exact TS mirror in feeMath.ts) covers BOTH the redemption split (weights = the vault's current protocol:manager composition) and the distribution split (weights = the pending-settlement ratio) with the same floor+exact-remainder, protocol-favored rounding convention as split_total_fee -- `protocolShare + managerShare == amount` always, by construction and by a 6-case exhaustive sweep test.\n\nDOCUMENTED, DELIBERATE IMPRECISION (never a fund-safety issue): `distribute_fee_usdc` resets the pending-settlement ratio to zero after any nonzero distribution rather than partially decrementing it, because there is no price oracle anywhere in this protocol and therefore no exact way to attribute a USDC amount that resulted from a DELAYED swap of an asset staged during an earlier, different-ratio redemption. Money always still reaches only the fixed Protocol Treasury and the Reserve's genuinely configured Manager recipient(s) -- the imprecision is bounded to which CYCLE'S ratio applies to a late-arriving swap, never who receives anything or how much total value moves.\n\nSECURITY: covered as new invariants 20-24 in docs/protocol/SECURITY_INVARIANTS.md -- bounded keeper blast radius (one asset's currently-staged amount, never Reserve principal), fixed/on-chain-configured destinations only, redemption reuses an already-proven entitlement formula, idempotent/duplicate-safe distribution, and the ProtocolConfig-never-grows rule. Off-chain keeper guards (MAX_PRICE_IMPACT_PCT, mirroring api/mainnet/jupiter-swap.ts) plus Jupiter's own on-chain otherAmountThreshold minimum-out enforcement cover slippage/route validation.\n\nMIGRATION/COMPATIBILITY: every currently-deployed Mainnet Reserve remains fully compatible, zero data migration -- Reserve/FeeConfig/TvlAccrual/ManagerFeeRecipients keep their exact current layout, and ProtocolConfig is untouched (the keeper lives in the new SettlementKeeperConfig singleton instead, precisely because of the bug caught above). collect_fees.rs/collect_manager_fee_share.rs/collect_protocol_fee.rs are completely unmodified -- any Reserve-Token-denominated pending balance that already exists today remains claimable exactly as before, indefinitely, matching this codebase's own established 'legacy drain, new path for new behavior' precedent. Only NEWLY crystallizing fees (after a future upgrade) route into the fee vault.\n\nCOST ESTIMATES: program grew 1,004,320 bytes total (from the 828,848-byte DEC-0114 baseline, i.e. this pass added ~171KB of new bytecode -- larger than this pass's own initial rough estimate of ~11KB, corrected here rather than left standing). New PDA rent, one-time per Reserve on first use: FeeSettlement (~90 bytes) + fee-vault ATA (~0.00203 SOL) + one staging ATA per distinct asset (~0.00203 SOL each) + one USDC staging ATA (~0.00203 SOL) -- roughly 0.01-0.02 SOL per Reserve total. Per-settlement-cycle costs: 3 ordinary Solana transactions (redeem/approve/distribute, ~5,000 lamports base fee each) plus the Jupiter swap's own ordinary transaction fee and whatever price impact/slippage the route carries -- no protocol-level fee from Jupiter itself.",
  "alternativesConsidered": [
    "Track per-batch (per-redemption-event) protocol:manager split with full historical precision, rather than a single running pending-settlement ratio that resets on distribution -- rejected as needing a much larger, per-asset, per-batch state structure (a fixed-size array bounded by max asset count times max concurrent batches) for a precision gain that doesn't change WHO receives funds or the protocol's aggregate correctness over time, only which specific settlement cycle's ratio a delayed swap's USDC gets attributed to. The simpler design was chosen deliberately and the resulting imprecision is explicitly documented rather than silently accepted.",
    "Redeem/settle per-asset immediately at crystallization time (no separate fee vault) -- rejected: violates the explicit requirement to keep settlement separate from accrual 'so small fees can accumulate until conversion is economically worthwhile,' and would mean every single mint/TVL-accrual event pays Jupiter swap costs for a potentially tiny amount.",
    "Let the keeper wallet directly custody redeemed assets outright (transfer instead of delegate-approve) -- rejected: a delegate approval keeps the program as the genuine owner of every asset until the instant a swap actually executes, strictly tighter custody than handing assets to an off-chain wallet outright, for the same operational effect.",
    "On-chain Jupiter CPI integration (the program itself calls into Jupiter's router) instead of an off-chain keeper-signed swap transaction -- rejected as a large, novel undertaking with no precedent anywhere in this codebase; the established, already-proven pattern throughout this app (seed-funding's own Jupiter integration) is client/keeper-built-and-signed swap transactions, which this pipeline reuses exactly."
  ],
  "impact": "Not deployed to Mainnet; no Mainnet program upgrade executed; no live settlement transaction of any kind submitted, per Creator's explicit instruction. Every currently-deployed Reserve is confirmed unaffected and remains fully compatible. 30 new regression tests (13 Rust -- cargo test -p ssr_protocol, covering split_by_weight exhaustively including zero-weight/all-one-side/rounding-exactness sweeps; 17 TypeScript -- tests/phase_fee_settlement_usdc.ts, covering the exact splitByWeight mirror, the keeper's isQuoteSafeToExecute price-impact guard, and composed scenarios for multi-event fee-vault accounting, idempotent/duplicate distribution, partial settlement across several assets, and multi-recipient Protocol/Manager distribution), 707/707 TypeScript offline tests passing (690 prior + 17 new), 13/13 Rust unit tests passing, `cargo check -p ssr_protocol`/`cargo clippy -p ssr_protocol` (zero new warnings)/`cargo-build-sbf` all clean, `npx tsc -b`/`npx oxlint`/`npm run build` all clean. IDL regenerated via the documented working `anchor idl build` command (space-free MinGW PATH fix, DEC-0113's precedent) rather than hand-patched -- caught and fixed two of its own real mistakes along the way (see evidence): a wrong assumption that the committed IDL should be camelCase (it's the raw snake_case/PascalCase anchor idl build output; only the separate .ts type-helper file is camelCase), and 2 test files with hardcoded pre-this-pass account-list/error-range expectations, both corrected to reflect the new, real, intentional shape (matching this codebase's own established 'confirmed correct and expected, not a regression' pattern for exactly this kind of test update).",
  "affectedAreas": [
    "programs/ssr_protocol/src/constants.rs",
    "programs/ssr_protocol/src/errors.rs",
    "programs/ssr_protocol/src/events.rs",
    "programs/ssr_protocol/src/fee_math.rs",
    "programs/ssr_protocol/src/state/fee_settlement.rs (new)",
    "programs/ssr_protocol/src/state/settlement_keeper_config.rs (new)",
    "programs/ssr_protocol/src/state/protocol_config.rs",
    "programs/ssr_protocol/src/instructions/mint_reserve_tokens_in_kind.rs",
    "programs/ssr_protocol/src/instructions/accrue_fees.rs",
    "programs/ssr_protocol/src/instructions/redeem_fee_vault_shares.rs (new)",
    "programs/ssr_protocol/src/instructions/approve_settlement_swap.rs (new)",
    "programs/ssr_protocol/src/instructions/distribute_fee_usdc.rs (new)",
    "programs/ssr_protocol/src/instructions/set_fee_settlement_keeper.rs (new)",
    "programs/ssr_protocol/src/instructions/common.rs",
    "programs/ssr_protocol/src/instructions/mod.rs, lib.rs",
    "packages/sdk/idl/ssr_protocol.json, ssr_protocol.ts",
    "packages/sdk/src/feeMath.ts, pda.ts, discovery.ts, readOnly.ts, feeSettlementInstructions.ts (new), index.ts",
    "api/mainnet/fee-settlement-cron.ts (new, NOT wired into vercel.json's crons)",
    "docs/protocol/ACCOUNT_MODEL.md, SECURITY_INVARIANTS.md",
    "docs/project/FEE_SETTLEMENT_RUNBOOK.md (new)",
    "tests/phase_fee_settlement_usdc.ts (new), phase_protocol_fee_auto_deposit.ts, phase_reserve_deploy_resumability.ts"
  ],
  "supersedes": null,
  "supersededBy": null,
  "evidence": [
    "cargo check -p ssr_protocol: clean. cargo clippy -p ssr_protocol: 3 pre-existing warnings only (update_protocol_config.rs redundant field names, common.rs needless_range_loop -- all predate this pass), zero new. cargo test -p ssr_protocol: 13/13 passing (including the caught-and-fixed split_by_weight rounding-direction bug -- first draft floored the wrong side, caught by its own test before this pass ever claimed success).",
    "cargo-build-sbf --manifest-path programs/ssr_protocol/Cargo.toml: clean release build, programs/ssr_protocol/target/deploy/ssr_protocol.so, 1,004,320 bytes.",
    "anchor idl build -p ssr_protocol (space-free MinGW PATH, DEC-0113's documented fix): succeeded, produced a genuine, freshly-compiled IDL -- diffed against the previously-committed one confirmed purely additive (33 instructions vs. 29, 8 accounts vs. 7, zero removed/renamed items) before being committed.",
    "Self-caught IDL mistake: initially applied convertIdlToCamelCase to the COMMITTED .json IDL (matching what seemed like DEC-0113's precedent) -- caused 20 test failures (account-list assertions expecting snake_case names, error-name assertions expecting PascalCase). Root-caused by inspecting the PRE-this-pass committed file directly: it was ALREADY raw/uncoverted (snake_case instructions, PascalCase accounts/errors) -- only the separate ssr_protocol.ts file is the camelCase type helper. Reverted to the genuine raw anchor idl build output for the .json file; all 20 failures resolved.",
    "npx tsc -b, npx oxlint (repo-wide): clean.",
    "npx ts-mocha -p ./tests/tsconfig.json -t 30000 tests/phase_*.ts: 707/707 passing (17 new in tests/phase_fee_settlement_usdc.ts; 2 pre-existing test files updated in place to reflect the new, real, intentional account-list shape and the error-code range growing from 6000-6056 to 6000-6061 -- both confirmed correct/expected, not regressions, matching DEC-0113's own identical precedent for an appended-error-range test update).",
    "npm run build: clean (SDK + frontend)."
  ]
}
```
...
```json
{
  ...
  "decision": "Implemented a real Mainnet USD pricing layer (Pyth Core primary, Jupiter Price V3 fallback, both server-side) and fixed the resulting ALPHA Reserve display bugs (Token Price/AUM/Market Cap reading $0, Buy 'Est. You Receive' reading ~0.00), plus two further ALPHA-specific correctness bugs surfaced while tracing the estimate against the real transaction. Removed the redundant 'NAV per Token' card, replaced with Market Cap. Deployed to production (https://ssr-fun.vercel.app) and live-verified against the real ALPHA Reserve.",
  "context": "Creator reported ALPHA (the real, live, 100%-SSR-backed Mainnet Reserve first surfaced in DEC-0131) showing 'Token Price $0'/'AUM $0' and 'Est. You Receive ~0.00 ALPHA' for a 10-unit Buy, despite a previous real purchase having produced ~9.90 ALPHA after the 1% Mint Fee. Creator specified the exact pricing architecture to build: Helius/RPC authoritative for on-chain state; Pyth Core server-side with verified mint->feed-ID mapping (never symbol-based), identity/timestamp/staleness/confidence validation, official Hermes endpoints, and explicit accounting for Pyth's announced 26 August 2026 Core upgrade; Jupiter Price API V3 server-side with an API key, mint/price/decimals/block-recency validation, brief timestamped caching, missing/null treated as unavailable never zero; a Pyth-then-Jupiter hierarchy with deviation flagging; AUM/Token Price/Market Cap formulas; removal of the NAV-per-Token card in favor of Market Cap; an honest 'Price unavailable' state instead of any fabricated $0; a full ALPHA diagnosis (vault/mint/decimals/supply, whether Pyth supports the SSR mint, manual AUM/price/market-cap arithmetic, and exactly where a real nonzero value becomes zero); a Buy/Sell estimate fix reconciled against the program; regression tests; and validate+commit+push+deploy with a live post-deploy ALPHA check. Explicit standing instruction: do not upgrade the Mainnet program; if on-chain Pyth consumption were required, document the exact program/IDL change and stop for approval.",
  "rationale": "DIAGNOSIS (live on-chain/API forensics, no assumptions): identified the real ALPHA Reserve as `H7NDKmf9pKow6v7eQfBchxRtPb8thP1STsRARGPWwo73` (reserveId 7, the only 'active', nonzero-supply, SSR-backed Reserve among 7 discovered on program `8hTW7fHwn8t8hcgTVeyAhHMiCTHGUP3783NWUTBBFwH9`; metadataUri resolves name/ticker to 'ALPHA'). Its sole registered asset is the real SSR mint `BpdHpqznEgYPXZNrJVRZvBhdWoafYLVVuLxTQo34pump` (decimals 6, NOT a devnet fixture or a different address than the app's constants believed), vault balance 21,132.446514 SSR (21132446514 raw), Reserve Token supply 9.95 (9950000 raw, RESERVE_TOKEN_DECIMALS=6), mintFeeBps 100 (1%, matching Creator's report), redemptionFeeBps 0. Live-queried Pyth Hermes (`/v2/price_feeds?query=...`, both by symbol and directly by the SSR mint address) and confirmed definitively: NO Pyth feed exists for SSR. Live-queried Jupiter Price API V3 (`api.jup.ag/price/v3?ids=<mint>`) and confirmed it DOES have a real, current price for SSR (~$0.00048 at diagnosis time, pump.fun-launchpad-tagged). Manual arithmetic with these real numbers: AUM = 21132.446514 x $0.00048 ~= $10.17-10.20 (varied slightly between the diagnosis pass and the post-deploy verification pass, consistent with SSR's own live price moving), Token Price (NAV) = AUM / 9.95 ~= $1.02-1.03, Market Cap = supply x Token Price ~= AUM (by construction, see below) -- all previously $0/~0 purely because of the root cause below, never because ALPHA genuinely lacked value.\n\nROOT CAUSE (exact): `src/merge/lib/onChainReserve.ts`'s `TEST_ASSET_PRICES_USD` -- a fixed, deliberately DevNet-fixture-only test-price table (documented as such in its own header) -- was ALSO being used, unconditionally, for every Mainnet asset's USD valuation. Any mint outside that table (every real Mainnet asset except real Circle USDC) priced as `?? 0` in both `mergeOnChainIntoDTR` and `buildDtrFromDiscoveredReserve`'s AUM loops, making `aumUsd`, then `nav` (`aumUsd / supply`), then `tokenPrice` (`= nav`) all deterministically 0 for ALPHA. DTRDetail.tsx's Buy estimate (`estReserveTokensOut = numBuyAmount / dtr.nav`) then divided by that same 0 (guarded to a flat 0, never NaN) -- exactly 'Est. You Receive ~0.00 ALPHA.' No real value was ever lost or miscounted on-chain; this was purely a missing real-pricing integration on the display layer, which never previously existed for anything beyond the DevNet fixture table.\n\nIMPLEMENTED: `api/mainnet/asset-prices.ts` (new, POST, rate-limited, batched up to 30 mints, briefly cached server-side) is the ONLY place a real price is ever fetched -- Pyth Hermes (`GET /v2/updates/price/latest`, live-verified request/response shape, verified feed IDs for real USDC/wrapped SOL hand-confirmed against Hermes's own `/v2/price_feeds` search before being hardcoded, since a wrong feed ID would silently price the wrong asset) as primary, Jupiter Price API V3 (`api.jup.ag/price/v3`, reusing the already-configured `JUPITER_API_KEY`) as fallback, resolved via `packages/sdk/src/pricing.ts`'s pure (network-free, independently unit-tested) validation/hierarchy logic: Pyth quotes rejected if stale (>60s), low-confidence (conf/price > 2%), or a feed-ID identity mismatch; Jupiter quotes rejected if `usdPrice` is null/non-positive, `decimals` doesn't match the on-chain-verified value, or `blockId` (a Solana slot) is too far behind the current slot (fetched once per request via `Connection.getSlot()`); a mint priced by neither source returns `{usdPrice: null, source: 'unavailable'}`, never a coerced 0; when both sources produce a valid quote, Pyth wins and a >1% relative disagreement is flagged (`deviationFlagged`) for future monitoring, never silently averaged. `PYTH_HERMES_BASE_URL` is an env override specifically so Pyth's announced 26 August 2026 Core upgrade can be absorbed as a config change if it moves/versions the Hermes endpoint, without touching code.\n\n`onChainReserve.ts`'s new `computeAumFromPrices` (pure, unit-tested) replaces the `TEST_ASSET_PRICES_USD` lookup for every Mainnet caller (DevNet is completely untouched -- it still uses the exact same fixed test table it always has, since Pyth/Jupiter have no knowledge of DevNet fixture mints anyway). AUM sums `vaultBalance x usdPrice` over every MATERIALLY-held asset (nonzero vault balance -- an asset the Reserve doesn't actually hold yet never blocks pricing for lacking a feed); if ANY materially-held asset can't be priced, the whole Reserve's AUM/Token Price/Market Cap report `pricingComplete: false` and the UI shows 'Price unavailable,' never a silently-partial or fabricated `$0`. A genuinely empty Reserve (no assets held) is honestly `$0`/`priceSource: 'none'`, distinct from a real pricing failure. `computeMarketCap` (new) computes Market Cap independently as `circulating supply x Token Price`, per Creator's explicit 'do not label AUM as Market Cap directly' instruction -- it is mathematically equal to AUM today specifically because Token Price IS this protocol's internal NAV (no secondary market exists yet, every Buy/Sell executes at NAV); this equality, and why it would diverge if that ever changes, is documented in the card's own InfoTip copy and in SSR_ARCHITECTURE.md's new section 8, not left as an unexplained coincidence.\n\nUI: removed the 'NAV per Token' card from DTRDetail.tsx, ManageDTR.tsx, and the Discover/Featured card's metric chip (`reserveCardProps.ts`), replaced with Market Cap in all three (internal NAV itself is fully retained and unchanged for premium/discount, chart-baseline, and redemption-entitlement math, exactly as Creator required). AUM/Token Price/Market Cap all route through a new `formatUsdcOrUnavailable` helper -- renders 'Price unavailable' instead of `$0.00` whenever `pricingComplete` is false. Token Price now shows a 'via Pyth'/'via Jupiter'/'via Pyth + Jupiter' source caption with a relative last-updated age.\n\nBUY/SELL ESTIMATE FIX: traced the full chain (NAV -> decimals -> raw units -> gross output -> Mint Fee -> net output -> displayed formatting) against the real submitted transaction, per Creator's explicit instruction. Root cause of the reconciliation gap: the real Mainnet Buy (`buildDirectMintInstructions`) is a proportional IN-KIND deposit of the Reserve's own single asset -- no oracle, no USD conversion at all -- but the DISPLAYED estimate was computed from `numBuyAmount / dtr.nav`, a USD-price-derived formula with no connection to the actual on-chain ratio math, and thus wrong whenever the (now-fixed) NAV was itself unavailable, AND structurally capable of disagreeing with the real transaction even once pricing works. Fixed by computing the estimate with the EXACT same two functions the real transaction builder uses -- `computeDirectReserveTokensRequested` (the ratio math `mint_reserve_tokens_in_kind` itself performs) then `computeNetMintOutput` (the same ceiling-rounded Mint Fee) -- against the same live vault balance/supply/fee bps, with zero USD-price dependency for this transaction-critical path. Returns `null` ('Quote unavailable', submission disabled) rather than a fabricated 0 whenever the Reserve isn't yet seeded or its asset data isn't resolved.\n\nTWO FURTHER LATENT BUGS caught while tracing this same chain, both fixed: (1) `handleBuyMainnet`/`handleSellMainnet` both hardcoded `[MAINNET_USDC_MINT]` as the ONLY candidate mint passed to `fetchReserveOnChain`, which resolves an asset ONLY if its mint is in that candidate list (its ReserveAsset PDA is derived per-candidate, never enumerated independently) -- for a genuinely non-USDC Reserve like ALPHA this would resolve ZERO assets and the transaction would fail outright with 'requires a single-asset Reserve; found 0 assets.' Fixed by passing the Reserve's own already-known asset mint list instead. (2) The Buy panel's currency label/'Your balance' balance-fetch/insufficient-balance check were all hardcoded to USDC (`SETTLEMENT_MINT`/`SETTLEMENT_SYMBOL`) regardless of what the Reserve's real deposit asset actually is -- for ALPHA this meant the UI told the user they were depositing USDC and checked their USDC balance, while the real transaction deposits SSR from their SSR-mint ATA. Fixed via a new `resolveBuyAsset` helper (Mainnet + non-pure-USDC Reserve -> the Reserve's real sole asset; every other case -> USDC/devUSDC unchanged) threaded through every balance-fetch/label/insufficient-balance site. Both bugs are directly implicated in the originally-reported ALPHA confusion and are exactly the kind of 'consistency between the displayed quote and submitted transaction' issue Creator asked to be traced.",
  "alternativesConsidered": [
    "Derive the Buy estimate from the newly-fixed USD Token Price instead of raw on-chain ratio math -- rejected: the real transaction has zero USD/oracle dependency (pure in-kind ratio math), so a price-derived estimate can never be guaranteed to match it exactly and would go dark during any pricing-source outage; the program-reconciled approach is both more correct and strictly more available.",
    "Treat a Pyth/Jupiter disagreement as a hard block on displaying a price -- rejected per the explicit requirement ('compare them and flag material deviation,' not 'refuse to price'); Pyth's hierarchy-winning price is still shown, with the disagreement surfaced as a flag for monitoring, not silently hidden or blocking the whole display.",
    "Silently drop an unpriceable material asset from AUM (partial sum) rather than marking the whole Reserve unavailable -- rejected per the explicit 'do not publish fabricated AUM' requirement: a partial sum that looks like a complete, confident total is a worse failure mode than an honest 'Price unavailable.'",
    "Leave the fetchReserveOnChain hardcoded-USDC-candidate and USDC-labeled-balance bugs unfixed as 'out of scope' (pure pricing-layer task) -- rejected: both are directly load-bearing for whether ALPHA's Buy flow displays/labels/gates correctly at all, and both were concretely implicated by tracing the exact chain Creator asked to be traced; leaving them would mean the estimate could be fixed while the underlying transaction path (or its balance/label consistency) remained broken for the very Reserve this task was about."
  ],
  "impact": "Deployed to production Mainnet (https://ssr-fun.vercel.app, Vercel deployment dpl_AXafDX85kYfh9rDWUTDQNSSaV1BM via `vercel --prod`). Live-verified post-deploy against the real ALPHA Reserve: curled the live `/api/mainnet/asset-prices` endpoint directly (confirmed real Pyth USDC pricing with a genuine cross-source deviation check, and real Jupiter SSR pricing), then re-ran the live discovery + pricing + AUM/Market Cap computation end to end against ALPHA's real, current on-chain state through the deployed production endpoint: vault 21,132.446514 SSR, supply 9.95 ALPHA, live price $0.0004813/SSR -> AUM $10.17, Token Price $1.0222, Market Cap $10.17 (equal to AUM, as designed) -- all previously $0. No Mainnet program or IDL change of any kind; pricing is entirely off-chain/display-layer, so the 'stop for approval if on-chain Pyth consumption is required' condition never triggered. 32 new/updated regression tests (tests/phase_mainnet_pricing.ts: 30 new, covering Pyth staleness/confidence/identity validation, Jupiter null-price/decimals-mismatch/block-recency validation, the Pyth-wins/deviation-flagging hierarchy, computeAumFromPrices against the real ALPHA numbers including partial-pricing-never-fabricates-$0 and genuinely-empty-Reserve-is-honest-$0 cases, computeMarketCap, the program-reconciled Buy estimate including a $10-equivalent-SSR-deposit scenario reproducing the reported '~9.90 ALPHA' order of magnitude, and source-text regressions for the removed NAV card/added Market Cap card/formatUsdcOrUnavailable wiring; tests/phase_featured_cards_and_rpc_redaction.ts: 1 updated to assert the new Market Cap metric instead of the removed NAV one), 737/737 offline TypeScript tests passing (705 prior + 32 changed/new), `npx tsc -b`/`npx oxlint`/`npm run build` all clean.",
  "affectedAreas": [
    "api/mainnet/asset-prices.ts (new)",
    "packages/sdk/src/pricing.ts (new), index.ts",
    "src/merge/lib/assetPricing.ts (new)",
    "src/merge/lib/onChainReserve.ts",
    "src/merge/lib/RealReserveSync.tsx",
    "src/merge/lib/calculations.ts",
    "src/merge/lib/reserveCardProps.ts",
    ...
    ".env.example",
    "docs/protocol/SSR_ARCHITECTURE.md",
    "tests/phase_mainnet_pricing.ts (new), phase_featured_cards_and_rpc_redaction.ts"
  ],
  "supersedes": null,
  "supersededBy": null,
  "evidence": [
    "Live Solana Mainnet RPC query (discoverAllReserves against program 8hTW7fHwn8t8hcgTVeyAhHMiCTHGUP3783NWUTBBFwH9): identified ALPHA as reserve H7NDKmf9pKow6v7eQfBchxRtPb8thP1STsRARGPWwo73 (reserveId 7), confirmed its sole asset is BpdHpqznEgYPXZNrJVRZvBhdWoafYLVVuLxTQo34pump (decimals 6), vault balance 21132446514 raw, Reserve Token supply 9950000 raw, mintFeeBps 100, redemptionFeeBps 0; metadataUri resolved name/ticker to 'ALPHA'.",
    "Live curl against https://hermes.pyth.network/v2/price_feeds: confirmed NO Pyth feed exists for SSR (empty result both by symbol search and by the exact mint address); confirmed and live-verified the real Crypto.SOL/USD (ef0d8b6fda2ceba41da15d4095d1da392a0d2f8ed0c6c7bc0f4cfac8c280b56d) and Crypto.USDC/USD (eaa020c61cc479712813461ce153894a96a6c00b21ed0cfc2798d1f9a9e9c94a) feed IDs directly from Hermes (not from memory -- an initial recalled SOL feed ID was off by one hex digit and caught by a live 'odd number of digits' parse error before being used).",
    "Live curl against https://api.jup.ag/price/v3: confirmed a real, current SSR price (~$0.00048, pump.fun-tagged) and confirmed the exact V3 response shape ({usdPrice, decimals, blockId, priceChange24h, liquidity, createdAt}) against a live response, not assumed from documentation.",
    "npx tsc -b: clean across the whole workspace (including api/mainnet's own tsconfig reference) after rebuilding packages/sdk's dist/ output.",
    "npx oxlint: clean (only pre-existing, unrelated warnings).",
    "npx ts-mocha -p ./tests/tsconfig.json -t 1000000 tests/phase_*.ts: 737/737 passing.",
    "npm run build: clean (SDK CJS build + tsc -b + vite build).",
    "npx vercel --prod --yes: Production deployment dpl_AXafDX85kYfh9rDWUTDQNSSaV1BM, READY, aliased to https://ssr-fun.vercel.app.",
    "Post-deploy live verification: curl POST https://ssr-fun.vercel.app/api/mainnet/asset-prices with ALPHA's real assets returned real Jupiter-sourced SSR pricing and real Pyth-sourced USDC pricing with a genuine cross-source deviation check (0.0136% observed, below the 1% flag threshold); a follow-up script re-fetched ALPHA's live on-chain state, called the live production pricing endpoint, and ran computeAumFromPrices/computeMarketCap end to end, confirming AUM/Token Price/Market Cap all report real nonzero values (~$10.17 AUM, ~$1.02 Token Price, ~$10.17 Market Cap) instead of the previously-reported $0."
  ]
}
```
...
```json
{
  ...
  "decision": "Fixed Create Reserve genuinely failing for any Reserve with roughly 7+ assets (\"Transaction too large: 1830 > 1232\", nothing created on-chain) by splitting createReserve + one initializeReserveAsset per asset across as many transactions as their real, measured byte size actually requires, instead of always bundling them into one -- and extended the existing resume-from-partial-failure logic to cover a Reserve that landed with only SOME of its assets registered so far.",
  "context": "Creator reported (road-to-mainnet MCR-01, 2026-08-24 09:45) trying to create a 10-asset Reserve (SOL, Fartcoin, STONK, Cupsey, aura, GIGA, RENDER, XST, PONKE, Bert, 10% each) and getting 'Deployment Failed (Step 1/2: Creating Reserve + registering assets...) Transaction too large: 1830 > 1232 -- confirmed nothing was created on-chain.' 1830/1232 is @solana/web3.js's own PACKET_DATA_SIZE assertion inside Transaction.serialize() -- a real, hard Solana wire-size ceiling, not an app-level bug in the traditional sense.",
  "rationale": "createReserveClient.ts's own header comment already documented this as a known, accepted tradeoff at design time (DEC-0031): createReserve and every initializeReserveAsset were deliberately combined into ONE transaction 'for the realistic 1-3 asset case' to save a wallet approval. That assumption is stale now that the Reserve Asset picker offers thousands of searchable tokens (DEC-0121) -- a creator selecting 7+ assets was always going to hit this ceiling. Critically, the on-chain program was ALREADY DESIGNED for exactly this: programs/ssr_protocol/src/state/reserve.rs's ReserveStatus doc comment says outright 'Tracks resumable multi-step creation' -- create_reserve once, then any number of initialize_reserve_asset calls (Created -> AssetsInitializing), each independently, before seed_reserve. The bug was purely client-side over-bundling, never a protocol limitation, so the fix is to actually use the resumability the program already supports rather than to cap how many assets a Reserve can hold. Implemented packInstructionsBySize (src/merge/lib/createReserveClient.ts): builds a real (unsigned) Transaction with the connected wallet as feePayer and a syntactically-valid dummy blockhash (a blockhash is always exactly 32 bytes regardless of value, so no RPC round trip is needed for sizing), measures its real compiled-message wire size, and greedily bins instructions into the fewest transactions that each stay under the ceiling -- WITHOUT ever reordering them, since createReserve must remain instruction 0 of batch 0 (every initializeReserveAsset depends on the Reserve it creates existing). createReserveOnChain now sends each batch sequentially (one wallet approval per batch) instead of one bundled signAndSend. Extended the existing resumability state machine (createReserveResume.ts's determineDeploymentResumePoint, originally built for DEC-0019/the 2026-08-17 seed-funding resumability pass) with a new resume-from-registration point: a Reserve with FEWER assets registered than expected, still in Created/AssetsInitializing status, is now a normal resumable state (register exactly the remaining assets, in the same order, then continue to funding/seeding as usual) -- previously ANY asset-count mismatch, including this now-normal one, was refused outright as unsafe. Genuinely anomalous mismatches (more assets registered than expected, or fewer but already past AssetsInitializing) still refuse to auto-resume, unchanged. PendingReserveDeploy now also persists each asset's weightBps (previously only mint/decimals/seedWeightFraction were saved) since resuming registration needs the exact original target weight, not just the seed split. Known, accepted gap: Manager Fee Recipients / Additional Managers instructions (bundled into the LAST batch when present) are not persisted in PendingReserveDeploy and are not automatically retried if that specific batch fails after earlier registration batches already landed -- the Reserve itself is never blocked on this; both can still be added afterward via Manage Reserve. This narrow edge case was accepted rather than adding a larger persisted-state/on-chain-existence-check mechanism, since it only affects the (already rarer) combination of a large asset count AND multiple fee recipients/delegates AND a failure specifically on the final batch.",
  "alternativesConsidered": [
    "Cap the number of assets selectable in Create Reserve (e.g. to 6) -- rejected: a product regression (the picker was deliberately widened to the full Jupiter catalogue in DEC-0121-0123), and doesn't fix the underlying issue for a manager who still wants a genuinely diversified basket.",
    "Bundle registration into fewer, larger transactions using Solana's Versioned/Address-Lookup-Table transactions instead of splitting into multiple legacy transactions -- rejected for this pass: real functional improvement but materially larger scope (ALT accounts must be created and warmed up first, adding its own new failure/resume states) for a live Mainnet money flow; legacy-transaction batching fixes the reported failure with a much smaller, more auditable change.",
    "Persist feeRecipients/additionalManagers in PendingReserveDeploy and add on-chain existence checks so a resume can also retry them -- deferred, not rejected: real correctness improvement, but the reported bug never involved fee recipients, and this adds meaningfully more surface area (new persisted fields, new on-chain reads) than the fix actually required; flagged as a known follow-up rather than done speculatively."
  ],
  "impact": "749/749 offline tests passing (741 prior + 8 new: packInstructionsBySize's real-shaped batching/ordering/never-drops tests, including a direct reproduction of the reported 1-create+10-register shape; determineDeploymentResumePoint's new resume-from-registration cases and a genuine-mismatch case for 'more registered than expected'). tsc -b, oxlint, npm run build all clean. Not yet live-verified against a real 10-asset Mainnet Reserve creation (no browser-automation/wallet tool in this environment, same documented gap as every prior UI pass) -- Creator's own retry of the exact reported flow is the next real-world confirmation. Also fixed the Review step's Wallet Cost Summary, which previously always claimed registration was '(combined into one transaction)' regardless of asset count (now says how many transactions registration will actually take when it's more than one) and its numTransactions/approval-count estimates (previously hardcoded to 2-3 regardless of asset count).",
  "affectedAreas": [
    "src/merge/lib/createReserveClient.ts",
    "src/merge/lib/createReserveResume.ts",
    "src/merge/pages/CreateDTR.tsx",
  ...
  "supersededBy": null,
  "evidence": [
    "Creator-supplied screenshot (Create Reserve, Review & Deploy step, 10 assets 10% each): red toast 'Deployment Failed (Step 1/2: Creating Reserve + registering assets...) Transaction too large: 1830 > 1232 -- confirmed nothing was created on-chain. Safe to retry.'",
    "node_modules/@solana/web3.js/lib/index.cjs.js:1882 -- assert(wireTransaction.length <= PACKET_DATA_SIZE, `Transaction too large: ${wireTransaction.length} > ${PACKET_DATA_SIZE}`), confirming this is the library's own hard-coded 1232-byte ceiling, not an app-level throw.",
    "programs/ssr_protocol/src/state/reserve.rs's ReserveStatus doc comment: 'Tracks resumable multi-step creation (see DEC-0019) ... Created -> AssetsInitializing -> Seeded -> Active', confirming the protocol was already designed for exactly this multi-transaction registration shape.",
    "749/749 offline tests passing, including a direct reproduction of the reported shape (createReserve + 10 initializeReserveAsset-equivalent instructions) genuinely requiring more than one packed batch."
  ]
}
```
...
```json
{
  ...
  "date": "2026-08-25",
  "status": "confirmed-implemented",
  "decision": "Fixed classifyCreateReserveError misclassifying a genuinely ambiguous (unresolved) RPC-confirmation timeout as \"deterministic\" whenever it reached the classifier wrapped inside a CreateReserveStepError -- the real shape CreateDTR.tsx's Resume catch block always receives, never the bare AmbiguousConfirmationError the pre-existing test suite exercised directly. Root cause: createReserveOnChain/resumeReserveDeploymentOnChain's 6 real catch-and-rewrap sites all constructed `new CreateReserveStepError(e instanceof Error ? e.message : String(e), step, addresses)`, discarding the original error's type/identity entirely. Added an optional `cause` parameter (the standard ES2022+ Error option) to CreateReserveStepError, threaded the originally-caught error through all 6 throw sites, and made classifyCreateReserveError classify against `e.cause` when present rather than the wrapper alone. Also gave the Resume panel's \"ambiguous\" toast its own non-destructive title/copy (\"Outcome unknown -- verifying\") instead of sharing \"Resume failed\"'s destructive styling with a genuine failure.",
  "context": "Creator retried CHARLIE's Resume again after DEC-0143 shipped and hit (correctly labeled \"Mainnet\" this time) \"Mainnet RPC could not confirm signature 5ZXVDEFL34cK9gQ69sn4c3n2LWi28wsfqCjw8NqNxY3iiwKXFBEbcFkiUMcu9WZaFv76FSeYoAcLpHqHr25iJ9eC within the verification window\" during Step 2/2 (Seeding Reserve), reported as being \"stuck in another Mainnet Reserve-launch/resume loop.\" The Resume panel presented this as a deterministic failure that \"will not resolve itself on a plain retry\" (relabeling the primary action \"Try again anyway\"), even though the real condition was an ordinary unresolved RPC timeout that a plain Resume click -- which always re-reads real on-chain state before submitting anything -- resolves safely.",
  "rationale": "Independently verified the reported signature against two authoritative Mainnet RPC sources (this app's own configured Helius Mainnet endpoint, and the public api.mainnet-beta.solana.com) via getSignatureStatuses (searchTransactionHistory: true) and getTransaction (commitment: finalized): both returned null/not-found on both providers, confirming the transaction genuinely never landed on-chain (ordinary transient network behavior under this codebase's deliberate skipPreflight + maxRetries:0 + no-auto-retry policy, not a symptom of anything DEC-0142's lookup-table logic did differently). Re-read CHARLIE directly on-chain: Reserve 6FN24ZMVZw8fAiQhvtdRCAMdLPbieXHjBx4Trw9NnYiQ, status assetsInitializing, assetCount 6 -- byte-for-byte unchanged from DEC-0142/DEC-0143's last confirmed state, nothing lost or duplicated. Traced the misleading messaging to classifyCreateReserveError (createReserveResume.ts): its `e instanceof AmbiguousConfirmationError` check can only ever match the exact instance signAndSend/signSubmitConfirmVersioned throw -- but createReserveOnChain/resumeReserveDeploymentOnChain's own catch blocks (all 6 real throw sites) always re-wrap whatever they caught into a CreateReserveStepError built from `e.message` alone before it ever reaches CreateDTR.tsx, so `instanceof AmbiguousConfirmationError` was already false by construction; AmbiguousConfirmationError's own message text (\"...within the verification window...\") also doesn't contain \"timeout\"/\"timed out\"/\"429\", and carries no decodable custom error code, so classification fell through every rule to the deliberately-conservative \"deterministic\" default. Fixed by giving CreateReserveStepError an optional `cause`, threading the original caught error through all 6 real throw sites, and unwrapping it first in classifyCreateReserveError. Also split the Resume-panel toast: an \"ambiguous\" classification now shows a non-destructive \"Outcome unknown -- verifying\" toast that explicitly says this was not a failure, while \"deterministic\"/\"retryable\" keep the existing destructive-styled \"Resume failed\" toast, which remains accurate for those classes.",
  "alternativesConsidered": [
    "Have classifyCreateReserveError import CreateReserveStepError and unwrap it by type -- rejected: createReserveResume.ts is imported BY createReserveClient.ts (which defines CreateReserveStepError), so importing it back would create a circular module dependency; the standard `.cause` property (present on any Error, no import needed) avoids that entirely.",
    "Add a dedicated toast variant/styling for \"ambiguous\" -- rejected: this app's toast component defines only \"default\"/\"destructive\" variants (src/merge/components/ui/toast.tsx), and CLAUDE.md's UI Baseline requires reusing existing design tokens/components rather than introducing new ones ad hoc; the existing \"default\" (non-destructive) variant plus distinct copy is enough to stop implying a terminal failure, without inventing a new visual pattern.",
    "Treat the repeat timeout as evidence DEC-0142's Address Lookup Table submission path is flaky and re-investigate it -- rejected once the signature was checked directly: it never landed on-chain at all, and CHARLIE's on-chain state is unchanged from DEC-0143's last confirmed read; nothing about the ALT path is implicated."
  ],
  "impact": "784/784 offline tests passing (5 new, in tests/phase_reserve_deploy_resumability.ts's new \"9b\" block: classifying a CreateReserveStepError wrapping an AmbiguousConfirmationError/a decodable custom error/a rate-limit error/no cause at all/a wallet rejection). tsc -b, oxlint, npm run build all clean. CHARLIE itself was NOT touched by this pass -- no transaction was resubmitted, no account was closed or modified; this is a client-side classification/copy fix only. Resume was already safe to click before this fix (it always re-reads on-chain state first) -- this pass corrects the MESSAGING that was actively discouraging that safe action, not the underlying resumability logic itself.",
  "affectedAreas": [
    "src/merge/lib/createReserveClient.ts",
    "src/merge/lib/createReserveResume.ts",
    "src/merge/pages/CreateDTR.tsx",
  ...
  "supersededBy": null,
  "evidence": [
    "Direct read-only Mainnet RPC check on signature 5ZXVDEFL34cK9gQ69sn4c3n2LWi28wsfqCjw8NqNxY3iiwKXFBEbcFkiUMcu9WZaFv76FSeYoAcLpHqHr25iJ9eC against two independent providers (this app's own configured Helius Mainnet endpoint, and the public api.mainnet-beta.solana.com): getSignatureStatuses (searchTransactionHistory: true) returned {value:[null]} on both; getTransaction (finalized) returned null on both -- confirms it never landed on-chain.",
    "programs/ssr_protocol/src/instructions/close_reserve.rs's own require!(reserve.status == ReserveStatus::WindDown) and initiate_wind_down.rs's require!(reserve.status == ReserveStatus::Active): confirms a Reserve stuck in AssetsInitializing (CHARLIE's real current status) has no on-chain path to WindDown/Closed at all under the currently-deployed program -- closing/reclaiming rent would require either completing seeding first (reaching Active) or a new on-chain instruction.",
    "createReserveClient.ts's CreateReserveStepError, before this fix: `constructor(message: string, step, addresses)` with no cause parameter at all, and all 6 real throw sites constructing it from `e instanceof Error ? e.message : String(e)` alone -- confirmed by direct source read.",
    "784/784 offline tests passing, including the 5 new regression tests added by this pass."
  ]
}
```
...
```json
{
  ...
  "date": "2026-08-25",
  "status": "confirmed-implemented",
  "decision": "Fixed two real, reproducible bugs in Mainnet seed-funding that were independently producing the same 'deterministic, will not resolve on retry' false alarm DEC-0144 fixed for the ambiguous-confirmation case: (1) a dust-sized remaining funding deficit could scale down to a Jupiter swap-quote amount so small Jupiter itself rejects computing a slippage threshold for it (confirmed live: \"Cannot compute other amount threshold, with amount 1 and slippageBps 150\"); (2) api/mainnet/jupiter-swap.ts's own real HTTP 429 rate-limit body text (\"Too many swap requests from this client...\") doesn't contain the exact phrase isRateLimitError checked for (\"too many requests\"), so it fell through to the deterministic default instead of being retried. Added isDustDeficit (pure, in createReserveResume.ts) so a dust-sized deficit against an asset the wallet already holds SOME real balance of is seeded with what's already held instead of attempting an unquotable swap, surfaced via the existing onSwapShortfall toast (now reworded to be accurate whether a swap executed or was skipped); broadened isRateLimitError to match any message containing both \"too many\" and \"requests\", not only the exact phrase.",
  "context": "Creator reported, while trying to deploy a new Reserve (\"DELTA\", 10 assets): 'when i try to launch again it asks me for more reserve asset purchasing touches of like .02 USDC of x token etc. -- its not a clean experience. if assets are in the wallet the resume function should just push', plus two live-pasted Resume failures during Delta's funding step showing exactly the dust-quote and rate-limit messages above. A direct read-only enumeration of every Reserve this manager wallet (6BjTPAWGjUYjL2Hrvz7iVmzWv8yKHNDqUAif5DEPWZen) owns on Mainnet (getProgramAccounts, memcmp on manager @ byte offset 17, dataSize 375) surfaced a materially larger picture than CHARLIE alone: 10 real Reserves total, 8 stuck at AssetsInitializing (reserve_ids 1-6 from 2026-08-20, CHARLIE/9 from 2026-08-24, and a brand-new reserve_id 10 -- DELTA -- created 2026-08-25 in this same session), only 2 (ALPHA/7, BETA/8) ever reaching Active. Every one of the 8 stuck Reserves is currently unclosable for the same reason as CHARLIE (close_reserve requires WindDown, reachable only from Active) -- flagged to the Creator directly, with an explicit request to stop starting new deploys until reviewed.",
  "rationale": "Traced 'Cannot compute other amount threshold...' to fundSeedAssetsIdempotent's deficit-funding path: scaleUsdcBudgetForDeficit deliberately floors a scaled-down deficit at 1 raw USDC unit ('never 0', so Jupiter's quote API isn't asked for a non-positive amount) -- but empirically Jupiter ALSO rejects amount=1 outright, for a different reason (can't compute a nonzero 150bps slippage threshold at that size), so the 'never 0' floor didn't actually make the amount quotable. A multi-asset Reserve retried many times (DELTA, 10 assets, itself following CHARLIE's own many retries) is exactly the shape that produces this: assets partially funded by earlier attempts end up sitting within a few cents of their target, and every subsequent Resume re-attempts a swap for that dust remainder and fails identically. Fixed by skipping the swap entirely once the scaled deficit budget is below a fixed $0.05 floor (DUST_DEFICIT_USDC_RAW) AND the wallet already holds some real balance for that asset (a genuinely empty asset still attempts its full swap regardless of size, so a real gap is never silently accepted as 'close enough') -- seeds with the real balance already held and reports the (usually negligible) gap through the SAME onSwapShortfall mechanism a real post-swap shortfall already uses, which CreateDTR.tsx already renders as a toast (no UI changes needed for the Creator's requested 'pop up about slight imbalances'). Traced 'Too many swap requests...' to jupiterSwapClient.ts's fetchJupiterSwapQuote throwing the server's own body.error text verbatim on a non-OK response (this codebase's established fetch-error-wrapping pattern, e.g. uploadReserveMetadata does the same), which discards the real HTTP 429 status entirely -- isRateLimitError's exact-phrase check ('too many requests') doesn't match api/mainnet/jupiter-swap.ts's own wording ('too many SWAP requests'). Fixed by requiring both words present (in either order, not necessarily adjacent) instead of the exact phrase, which now correctly classifies this as 'retryable' end-to-end through classifyCreateReserveError (already unwrapping CreateReserveStepError's cause since DEC-0144) without needing a second, narrower special case.",
  "alternativesConsidered": [
    "Widen slippageBps for a dust-sized swap instead of skipping it -- rejected: doesn't actually fix the underlying 'Cannot compute other amount threshold' failure mode (Jupiter's rejection is about the absolute amount being too small to produce a nonzero threshold at all, not about the slippage percentage chosen), and adds real execution-price risk for no benefit over simply accepting a few cents of shortfall.",
    "Retry the deficit quote with progressively larger scaled amounts until Jupiter accepts one -- rejected: adds real complexity and extra Jupiter API calls (the exact thing tripping the OTHER bug this pass fixes, the rate limit) to chase a few cents that the Creator explicitly said isn't worth doing (their own proposed pragmatic fix: just push with what's already held).",
    "Match isRateLimitError only against literal 'too many swap requests' (the one observed string) -- rejected: narrower than necessary and would miss the next differently-worded rate-limit message from any other endpoint; requiring both words present is general enough to catch this class of message going forward without over-matching (regression tests confirm 'too many' alone and 'requests' alone, unpaired, still don't match)."
  ],
  "impact": "791/791 offline tests passing (9 new: 4 for isDustDeficit, 3 for isRateLimitError's broadened matching, plus the 2 already added by DEC-0144 remain). tsc -b, oxlint, npm run build all clean. Neither CHARLIE nor DELTA was touched by this pass -- no transaction was resubmitted; this is a client-side funding-logic and classification fix only. Flagged directly to Creator: 8 real Mainnet Reserves (not just CHARLIE) are currently stuck at AssetsInitializing and unclosable under the deployed program -- reviewing/deciding what to do with reserve_ids 1-6 is a separate, not-yet-actioned follow-up.",
  "affectedAreas": [
    "src/merge/lib/createReserveClient.ts",
    "src/merge/lib/createReserveResume.ts",
    "src/merge/pages/CreateDTR.tsx",
  ...
  "supersededBy": null,
  "evidence": [
    "Creator-pasted live Resume failure text: \"Cannot compute other amount threshold, with amount 1 and slippageBps 150\" during Delta's 'Funding seed assets (Mainnet)' step, and separately \"Too many swap requests from this client -- wait a moment and try again.\"",
    "api/mainnet/jupiter-swap.ts:73 -- `res.status(429).json({ error: \"Too many swap requests from this client -- wait a moment and try again.\" })`; jupiterSwapClient.ts:69 -- `throw new Error((body && typeof body.error === \"string\" && body.error) || ...)`, confirming the HTTP 429 is discarded and only this body text reaches isRateLimitError.",
    "Direct read-only getProgramAccounts enumeration of manager 6BjTPAWGjUYjL2Hrvz7iVmzWv8yKHNDqUAif5DEPWZen against the live Mainnet ssr_protocol program: 10 real Reserve accounts, 8 at AssetsInitializing (reserve_ids 1-6, 9=CHARLIE, 10=DELTA), 2 Active (7=ALPHA, 8=BETA).",
    "791/791 offline tests passing, including the 9 new regression tests added by this pass."
  ]
}
```
...
```json
{
  ...
  "date": "2026-08-25",
  "status": "confirmed-implemented",
  "decision": "Added bounded retry (3 attempts, short backoff) to api/mainnet/jupiter-swap.ts's own Jupiter quote-fetch call, specifically for the two failure shapes that indicate a transient upstream hiccup rather than Jupiter's own genuine answer: the fetch() call itself throwing, or a non-OK response whose body isn't parseable JSON with a real `.error` string. A genuine, parseable Jupiter error (its real shape for an actual no-route/not-tradable answer) is still reported immediately, verbatim, never retried. On exhausting all attempts without ever reaching Jupiter, the fallback message now says 'network error' explicitly (previously implied it via a generic 'No Jupiter swap route is currently available' wording that read as a permanent, asset-specific verdict) -- deliberately worded to match the substring classifyCreateReserveError already treats as retryable, so the client correctly stops claiming a plain retry can't help.",
  "context": "Immediately after DEC-0145 shipped, Creator's next Resume attempt on DELTA (the same real 10-asset Mainnet Reserve) hit a NEW failure: \"No Jupiter swap route is currently available for this asset\", again framed as deterministic (\"this specific failure will not resolve itself on a plain retry\"). Rather than accept the message at face value, independently tested every one of DELTA's 10 registered assets (enumerated directly on-chain via getProgramAccounts filtered on DELTA's PDA, decoding each ReserveAsset account) against Jupiter's own public quote API at $1/$50/$200/$500 notional -- every single one (9 swap-eligible, 1 wrapped SOL funded via a separate non-Jupiter path) returned a real, healthy route with negligible (<0.03%) price impact, at every size tested. This directly contradicted the 'no route' framing and pointed at a transient failure in this endpoint's own upstream call instead.",
  "rationale": "Read api/mainnet/jupiter-swap.ts's actual error-handling: the generic 'No Jupiter swap route is currently available for this asset' fallback fires whenever Jupiter's quote endpoint returns a non-OK response AND the body doesn't parse as JSON with a `.error` string -- NOT specifically when Jupiter says there's no route. Confirmed Jupiter's real shape for a genuine no-route/not-tradable answer by probing its public quote API directly with an intentionally-untradable mint (the System Program address): a normal 400 with a parseable body, `{\"error\":\"The token ... is not tradable\",\"errorCode\":\"TOKEN_NOT_TRADABLE\"}`. An UNPARSEABLE body (or the fetch() call throwing outright, the separate outer-catch path with its own generic 'Jupiter quote request failed.' message) is therefore much more consistent with an upstream gateway/network-level hiccup than a genuine semantic answer from Jupiter -- exactly matching the live evidence (every asset independently verified to have a healthy route moments later). Fixed by retrying only those two specific shapes (never retrying a genuine parseable Jupiter answer, which is likely stable and more informative to just show), and by wording the final give-up message to include 'network error' so it's classified consistently with every other transient-failure message in this app (DEC-0144/0145's same classifyCreateReserveError/isRateLimitError pattern), rather than inventing a third special case.",
  "alternativesConsidered": [
    "Broaden classifyCreateReserveError further to also recognize 'no Jupiter swap route' as retryable -- rejected: a GENUINE 'no route' answer (Jupiter's own specific reason, e.g. TOKEN_NOT_TRADABLE) really can be a stable, non-retryable condition; blindly reclassifying the generic fallback text as retryable would be correct for THIS incident but wrong in general. Fixing the server to stop conflating 'Jupiter said no' with 'something upstream broke' is the more correct, durable fix.",
    "Retry indefinitely / with a longer bound -- rejected: 3 attempts with a short (400ms/800ms) backoff is enough to ride out a brief upstream blip (this pass's own regression test reproduces exactly a 2-failures-then-success pattern) without meaningfully slowing down a Creator's Resume click or risking this endpoint's own rate-limit budget (DEC-0145's 12-requests/60s window) against Jupiter.",
    "Retry the WHOLE handler (quote + swap-transaction build) as one unit instead of just the quote fetch -- rejected: unnecessary scope: the live incident and every piece of evidence point specifically at the quote call; the swap-transaction build step (a separate Jupiter endpoint) was never implicated and doesn't need its own retry loop added speculatively."
  ],
  "impact": "794/794 offline tests passing (3 new, mocking global.fetch to reproduce a genuine-Jupiter-answer/immediate-report case, a transient-then-recovers case, and a bounded-exhaustion case). tsc -b, oxlint, npm run build all clean. Neither CHARLIE nor DELTA was touched -- server-side retry/messaging fix only, no wallet signature involved at this step at all (quote-fetching is read-only). Confirmed live: every one of DELTA's 10 assets has a real, healthy Jupiter route right now -- Creator's next Resume click is expected to succeed (or, if it hits the SAME transient condition again, now retries automatically up to 3 times before ever surfacing an error).",
  "affectedAreas": ["api/mainnet/jupiter-swap.ts", "tests/phase_mainnet_production_fixes.ts", "docs/project/PROJECT_STATUS.md"],
  "supersedes": null,
  "supersededBy": null,
  "evidence": [
    "Direct read-only getProgramAccounts enumeration of DELTA's ReserveAsset accounts (memcmp on `reserve` @ byte offset 8, DELTA's PDA uA3sYDWvyPwkEmWGZmKgcRVGLPSRq6FbjnJrrHcWD7z): 10 assets (orderIndex 0 = wrapped SOL, 1-9 = real SPL mints), each 1000bps (10%) weight.",
    "Direct read-only Jupiter public quote API check (lite-api.jup.ag/swap/v1/quote) for every one of DELTA's 9 swap-eligible assets at $1/$50/$200/$500 USDC notional: all 36 checks returned a real route with <0.03% price impact -- no evidence of any genuine no-route condition for any asset.",
    "Direct probe of Jupiter's real error shape for a genuinely untradable mint (the System Program address, 11111111111111111111111111111111): HTTP 400, body `{\"error\":\"The token 11111111111111111111111111111111 is not tradable\",\"errorCode\":\"TOKEN_NOT_TRADABLE\"}` -- confirms a genuine no-route answer IS parseable JSON with a specific message, unlike the generic fallback this pass fixes.",
    "794/794 offline tests passing, including the 3 new regression tests added by this pass."
  ]
}
```
...
```json
{
  ...
  "date": "2026-08-25",
  "status": "confirmed-implemented",
  "decision": "Widened the seed-funding skip condition from an absolute dust-dollar floor (DEC-0145) to ALSO skip a Jupiter swap once the wallet's existing balance is within ordinary market-drift tolerance (5%, the same SHORTFALL_WARN_PCT already used post-swap) of a freshly re-quoted target -- fixes a Creator-reported 'still prompted me to buy tokens although i have in wallet' report. Also cached the FULL-budget Jupiter quote (used only to determine the live-priced target, never the actual swap quote executed) for 20 seconds via this app's existing getCached utility, keyed by mint+budget -- directly reduces how many real Jupiter-quote requests a Creator retrying Resume a few times in quick succession generates, a real, self-inflicted contributor to the recurring 'Too many swap requests' rate limit (DEC-0145's own api/mainnet/jupiter-swap.ts 12-requests/60s-per-IP budget).",
  "context": "Immediately after DEC-0146 shipped, Creator reported DELTA's Resume 'still prompted me to buy tokens although i have in wallet. and then hit the too many swap requests error.' -- i.e. an asset the Creator plainly already held enough of was still triggering a wallet swap-approval popup, and a subsequent attempt hit the same rate limit DEC-0145 already investigated once.",
  "rationale": "Re-read fundSeedAssetsIdempotent's own logic: `targetRaw` is NOT a fixed number -- it's `fullQuote.outAmount`, a LIVE Jupiter quote for the asset's full USD budget, refetched fresh on every single Resume click. For a volatile/thin-liquidity token, real price movement between attempts can make a wallet balance that was already genuinely adequate (from an earlier attempt, acquired at a different price) look 'short' again on a later click -- DEC-0145's isDustDeficit only skipped an absolute few-cents gap, not a real few-dollar gap explained entirely by ordinary price drift, so this case still fell through to a real swap attempt and wallet prompt. Fixed with isWithinAcceptableShortfallTolerance: skip the swap whenever the existing balance is already within the SAME 5% tolerance this app already treats as 'not even worth warning about' once a swap executes (SHORTFALL_WARN_PCT) -- reused rather than inventing a new threshold, and only ever applies when the wallet holds SOME real balance already (never lets a genuinely empty asset skip funding). Separately, re-examined why the rate limit kept recurring: fundSeedAssetsIdempotent fetches the FULL-budget quote for EVERY swap-eligible asset on EVERY call, unconditionally -- for DELTA's 9 swap-eligible assets, that's 9 real requests against this app's own 12-requests/60s-per-IP proxy limit on EVERY Resume click, before any deficit/swap logic even runs. A Creator retrying Resume 2-3 times in a short debugging window (exactly what's been happening across DEC-0144 through DEC-0146) can exhaust that budget on quote-checking alone, with zero swaps actually needed. Wrapped this specific call in getCached (already used elsewhere in this app for the identical 'don't ask the same question twice within a few seconds' purpose, e.g. rent constants) with a 20-second TTL -- safe here specifically because this cached value is ONLY used to determine the comparison target, never as the actual quote a swap transaction is built from (that remains a fresh, uncached call, unchanged).",
  "alternativesConsidered": [
    "Only fix the tolerance issue, treat the rate limit as already addressed by DEC-0146's retry logic -- rejected: DEC-0146's retry logic helps a single request survive a transient Jupiter-side hiccup, but does nothing about the REQUEST VOLUME this app's own code generates per Resume click, which is the actual, separate driver of hitting our own proxy's rate limit on repeated clicks.",
    "Raise api/mainnet/jupiter-swap.ts's own rate-limit ceiling (currently 12 requests/60s per IP, DEC-0145) instead of reducing request volume -- rejected: loosens a deliberate abuse/cost-control safeguard for every caller, not just this one legitimate-but-chatty flow; reducing the actual number of requests this flow needs is the more targeted fix and has no such tradeoff.",
    "Use a longer cache TTL (e.g. 60s+) for the full-budget quote -- rejected: 20s is enough to collapse a burst of Resume clicks within a short debugging window while staying reasonably fresh against real price movement for the (separate, still-live) tolerance check; a much longer TTL would let a stale price stand in for the 'is this close enough' decision for longer than seems prudent for a real fund-safety-adjacent comparison."
  ],
  "impact": "798/798 offline tests passing (4 new: isWithinAcceptableShortfallTolerance's pure decision logic). tsc -b, oxlint, npm run build all clean. Neither CHARLIE nor DELTA touched -- client-side funding-logic and caching fix only. Creator's next Resume click on DELTA should no longer prompt a swap for an asset already held within normal price-drift tolerance, and should make meaningfully fewer real Jupiter-quote requests if retried again shortly after.",
  "affectedAreas": ["src/merge/lib/createReserveClient.ts", "src/merge/lib/createReserveResume.ts", "tests/phase_reserve_deploy_resumability.ts", "docs/project/PROJECT_STATUS.md"],
  "supersedes": null,
  "supersededBy": null,
  "evidence": [
    "Creator-reported: \"still prompted me to buy tokens althought i have in wallet. and then hit the too many swap requests error.\"",
    "createReserveClient.ts's fundSeedAssetsIdempotent, before this fix: `const fullQuote = await fetchJupiterSwapQuote(...)` called unconditionally for every swap-eligible asset on every invocation, with no caching -- confirmed by direct source read; for DELTA's 9 swap-eligible assets, 9 real requests per Resume click against api/mainnet/jupiter-swap.ts's 12-requests/60s-per-IP limit.",
    "798/798 offline tests passing, including the 4 new regression tests added by this pass."
  ]
}
```
...
```json
{
  ...
  "date": "2026-08-25",
  "status": "confirmed-implemented",
  "decision": "First-principles correction of Mainnet Reserve inception and purchasing, replacing per-error patching with a coherent system: (1) established the binding product funding invariant -- a user supplies ONLY Mainnet USDC (plus SOL for fees/rent); inception is USDC -> Jupiter swaps -> all Reserve Assets -> seed -> mint, purchase is USDC -> swaps -> deposit -> mint, for EVERY Reserve including 100%-single-asset ones; a resume counts already-held confirmed balances but only obtains genuine remaining deficits and never repeats successful work -- documented in docs/protocol/FRONTEND_INTEGRATION.md's new 'Mainnet funding invariant' section. (2) Built an explicit, persistent per-asset launch state machine (src/merge/lib/launchFunding.ts: not_started -> quoted -> awaiting_signature -> submitted -> confirmed -> balance_verified -> ready_to_seed, persisted inside PendingReserveDeploy.assetFunding), with seeding hard-gated on EVERY asset being ready_to_seed -- completion never inferred from wallet approval, submission, optimistic UI, or timeout. (3) Added an upfront launch-feasibility preflight (assessLaunchFeasibility) run in CreateDTR.tsx BEFORE the Reserve PDA is created: rejects dust-sized designed allocations with a precise computed minimum recommended seed (min practical per-swap USD / smallest USDC-consuming weight fraction -- never an arbitrary blanket number) and rejects a plan the wallet's real USDC balance cannot fund, with the exact shortfall. (4) Reworked fundSeedAssetsIdempotent: per-swap USDC preflight immediately before requesting a signature; a FRESH execution quote for every swap (the 20s-cached quote is now used STRICTLY for target estimation, never executed -- a previously-shipped bug had the cached quote executed when the wallet held none of an asset); reconciliation of any previously-submitted persisted signature against real getSignatureStatuses before any new swap; per-asset progress ('N of M assets funded') surfaced in CreateDTR's step label. (5) Corrected Jupiter on-chain rejection decoding to Jupiter's DOCUMENTED codes: 6024 = InsufficientFunds (deterministic, message shows held vs required and says retrying without funding fails identically -- partially reverting DEC-0150's blanket 'swap rejections are retryable', which was wrong for this code), 6001 = SlippageToleranceExceeded (retryable). (6) Fixed Reserve purchasing to honor the invariant: DTRDetail routes EVERY Mainnet Reserve except pure-USDC through the USDC-funded buy path (multiAssetBuyClient works identically for 1 leg), so ALPHA (100% SSR) is now bought with USDC -- previously it demanded the buyer already hold SSR itself; resolveBuyAsset now always returns the settlement asset, and the quote estimate for USDC-funded buys is NAV-based, mirroring exactly what the submitted transaction requests. NO on-chain program change was needed -- the deployed program already supports the full flow.",
  "context": "Creator escalated after DEC-0144..0150's sequence of individually-correct-but-piecemeal fixes: 'Stop patching individual error messages. Establish the complete intended flow, find every violated invariant, and fix the system coherently.' The triggering failure: a ~$20 ten-asset Mainnet launch repeatedly failing around the fourth swap, with some approvals succeeding; the Creator believed the flow moved into seeding prematurely, and reported an asset displayed as 'ETH'.",
  "rationale": "Evidence-first reconstruction before any code: (a) getSignaturesForAddress on the manager wallet showed the exact failure signature -- successes at 12:33:57-12:34:41 (creating Reserve 11, EQ5HNT9hAnYFfXARRJeKrNahMLaag5tepXV1uoK6CqoM, 10 assets), then FIVE consecutive failures (12:34:50, 12:35:21, 12:40:27, 12:52:12, 13:08:21), all InstructionError[3, Custom 6024]. (b) getTransaction on the failed swap: the Jupiter aggregator program itself (JUP6LkbZ...) failed after only 1,884 compute units at SharedAccountsRoute entry -- input validation, NOT slippage (slippage fails at route end); output mint USD1ttGY... (USD1, World Liberty Financial USD, Reserve 11's orderIndex-6 asset); the wallet's USDC balance visible in the same transaction's preTokenBalances: 0.490579 USDC against a ~$2 required input. (c) Jupiter's own documentation (developers.jup.ag/docs/swap/common-errors): 6024 = InsufficientFunds, recommended handling 'show the user the current balance and the required balance' -- exactly what nothing in the app did. (d) Reserve 11's swap order (SSR held/skipped, PENGU ok, HYPE ok, ZEC ok, USD1 = 4th swap) matches the Creator's 'failed around the fourth swap' precisely; the wallet's PENGU/HYPE/ZEC balances confirm swaps 1-3 succeeded. (e) The 'ETH' asset does not exist in either launch: Jupiter tokens-v2 metadata identifies every registered mint -- DELTA held ZEC (A7bdiYdS5G..., Zcash, 8dp) among others; Reserve 11 holds SSR/USDC/SOL/PENGU/HYPE(98sMhvDwXj..., 9dp)/ZEC/USD1/CARDS(Collector Crypt)/MET(Meteora)/jlUSDC -- all classic token program, all Jupiter-verified, all liquid (Token-2022 and no-route causes ruled out). The USDC ran out because earlier abandoned launches (now 9 stuck Reserves of 11 total for this wallet) had already converted it into OTHER constituent tokens each new composition didn't reuse. Premature-seeding: seeding was never actually reached in these launches (funding threw first), but the gate previously was only implicit-by-throw -- the explicit canEnterSeeding gate makes it structural. $20/10 assets verdict: VIABLE on the allocation axis ($2/asset clears the $0.50 practical per-swap floor and every protocol raw-unit minimum by orders of magnitude) but with near-zero headroom -- the computed practical minimum for a 10x10% composition is $5.00 seed on the allocation axis, and the wallet must hold the full swap-funded total (~$18.54 for a fresh $20 launch: 9 USDC-consuming legs x $2 x 1.03 buffer) plus SOL for fees/rent.",
  "alternativesConsidered": [
    "A program upgrade adding an on-chain per-asset funding record -- rejected: the deployed program's Created -> AssetsInitializing -> Active machine plus atomic seed_reserve already enforces the on-chain half (nothing can partially seed); the violated invariants were all client-side (no feasibility check, no persistent per-asset state, cached execution quotes, untruthful error decoding, single-asset buys demanding the underlying asset). No protocol change is needed for the intended flow, so none is proposed.",
    "Keep DEC-0150's blanket 'on-chain swap rejections are retryable' and special-case only the message text -- rejected: classification must follow Jupiter's documented code semantics; 6024 is deterministic (fund the wallet), 6001 is retryable, and conflating them is exactly what produced five doomed retries in 34 minutes.",
    "A blanket minimum launch amount (e.g. '$50 minimum') -- rejected per the Creator's explicit instruction: the preflight computes the per-composition minimum from the smallest USDC-consuming weight fraction and the practical per-swap floor, and reports the precise number.",
    "Auto-selling the stranded constituent tokens from abandoned launches back to USDC to fund the next launch -- deliberately NOT built: an on-chain value-moving action taken without explicit instruction; the tokens remain in the wallet, counted toward any future launch that includes the same mints."
  ],
  "impact": "828/828 offline tests passing (27 new/updated: tests/phase_launch_funding_state_machine.ts covers the full lifecycle, illegal-transition refusal, retry reset, delayed confirmation, the seeding gate incl. the exact failed-fourth-swap shape, ten-asset/$20/dust/insufficient-USDC feasibility incl. the literal 0.490579-USDC live case, SOL-leg exclusion, mint-not-symbol identity, and the ALPHA USDC-purchase math; phase_reserve_deploy_resumability's 9c block rewritten for documented-code classification). tsc -b, oxlint, npm run build all clean. No secrets in the diff; no DevNet/Mainnet mixing (cluster-gated paths unchanged in shape). Reserve 11 and DELTA were NOT touched (per instruction); no on-chain state was modified by this pass. Deployed to production.",
  "affectedAreas": ["src/merge/lib/launchFunding.ts", "src/merge/lib/createReserveClient.ts", "src/merge/lib/createReserveResume.ts", "src/merge/lib/jupiterSwapClient.ts", "src/merge/pages/CreateDTR.tsx", "src/merge/pages/DTRDetail.tsx", "docs/protocol/FRONTEND_INTEGRATION.md", "tests/phase_launch_funding_state_machine.ts", "tests/phase_reserve_deploy_resumability.ts", "docs/project/PROJECT_STATUS.md"],
  "supersedes": "DEC-0150 (partially: 6024's classification)",
  "supersededBy": null,
  "evidence": [
    "getSignaturesForAddress(6BjTPAWG...): five consecutive InstructionError[3,{Custom:6024}] failures 12:34:50-13:08:21 UTC 2026-08-25, immediately after Reserve 11's creation burst at 12:33:57-12:34:41.",
    "getTransaction(28ezn3tB...): JUP6LkbZ... failed at 1,884 CUs into SharedAccountsRoute (entry validation, not slippage); ATA createIdempotent for mint USD1ttGY...; preTokenBalances show the wallet's USDC ATA at 0.490579.",
    "Jupiter documentation (developers.jup.ag/docs/swap/common-errors): 6024 = InsufficientFunds, 'Insufficient funds for either swap amount, transaction fees or rent fees'; recommended handling: show current vs required balance.",
    "Jupiter tokens-v2 metadata for every mint in DELTA and Reserve 11: the reported 'ETH' is ZEC (A7bdiYdS5GjqGFtxf17ppRHtDKPkkRqbKtR27dxvQXaS, Zcash, 8 decimals, classic token program, Jupiter-verified); no ETH-denominated asset is registered in either launch. Reserve 11's full identity: SSR, USDC, SOL, PENGU (2zMMhcVQ...), HYPE (98sMhvDwXj...), ZEC, USD1 (USD1ttGY...), CARDS (CARDSccUM..., Collector Crypt), MET (METvsvVR..., Meteora), jlUSDC (9BEcn9aP..., Jupiter Lend USDC).",
    "getProgramAccounts enumeration: 11 Reserves under this manager, 9 stuck AssetsInitializing (ids 1-6, 9=CHARLIE, 10=DELTA, 11), 2 Active (7=ALPHA, 8=BETA).",
    "Wallet token balances confirming swaps 1-3 of Reserve 11 succeeded before the 4th failed: PENGU 522.199482, HYPE 0.062719594, ZEC 0.00600539.",
    "828/828 offline tests passing, including the 27 new/updated regression tests."
  ]
}
```
...
```json
{
  "id": "DEC-0154",
  "date": "2026-08-25",
  "status": "confirmed-implemented (NOT deployed -- per instruction: no push/deploy this pass)",
  "decision": "Root-caused and fixed the Mainnet Reserve Token buy/mint flow from first principles. (1) ROOT CAUSE: the SDK-embedded IDL (packages/sdk/idl/ssr_protocol.*, regenerated 2026-08-21 10:08 during the DEC-0133 fee-settlement pass) changed mint_reserve_tokens_in_kind's and accrue_fees' account lists to the never-deployed fee-settlement source shape, while the DEPLOYED Mainnet binary (last deployed 2026-08-19T10:34:43Z, slot 440248996, NEVER upgraded since -- read from the BPFLoaderUpgradeable programdata account) still expects the pre-fee-settlement shape (source commit c2b84ba). Every mint built since 2026-08-21 sent 13 accounts in the wrong composition; on-chain, the token program slid into the slot the deployed binary reads as manager_fee_recipients, failing with Anchor 3002 AccountDiscriminatorMismatch ('caused by account: manager_fee_recipients' in the transaction logs). Restored both instructions' account lists in the SDK IDL (JSON + TS twin) to the deployed shape and reverted the builders accordingly; PROVEN by read-only Mainnet simulation of the corrected CHARLI mint (sigVerify off, nothing landed): err null, full success, mint+fee-transfer inner CPIs executing. (2) The SOL->WSOL mystery: the buy's first wallet transaction wrapped ~$5 of the buyer's NATIVE SOL because multiAssetBuyClient funded a wrapped-SOL Reserve asset leg by wrapping the buyer's own SOL -- violating the DEC-0151 funding invariant while the UI named USDC as the deposit asset. Fixed: every leg including wrapped SOL now funds via a Jupiter swap FROM USDC (new receiveWrappedSol flag in api/mainnet/jupiter-swap.ts sets wrapAndUnwrapSol:false so the output stays SPL wrapped SOL); already-held balances (including the stranded 0.05151 wSOL from the failed attempt) always count first. (3) New pure planning/safety module multiAssetBuyPlan.ts: planBuyFunding (USDC-only actions, deficit-only, never repurchases a held leg), assessBuyFeasibility (whole-purchase USDC + SOL-for-fees gate BEFORE any transaction, exact current-vs-required), shouldSubmitMint (double-mint guard from the buyer's REAL Reserve Token balance), buildBuyStateReport (post-failure per-leg held/required/where + minted-or-not + what-retry-does, from freshly-read balances only). (4) multiAssetBuyClient rework: per-leg persistent state (launchFunding's machine under ssr_pending_buy_v1, wallet+reserve-keyed), previously-submitted swap/mint signatures reconciled against real status before any resubmission, delivery verification (success only after the buyer's real Reserve Token balance shows the mint), decoded on-chain errors via describeOnChainError, targeted [multi-asset-buy] logging (deposit asset, quotes, routes' in/out amounts, signatures, per-leg states, vault-before figures, RT supply + buyer ATA before/after, retry decisions). (5) DTRDetail: synchronous duplicate-click ref guard on both buy paths; failure toasts render the on-chain-verified state report, never a blanket funds-are-safe claim; an alreadyMinted result is reported as already-completed, nothing re-charged.",
  "context": "Creator attempted a ~$10 USDC purchase of CHARLI (Reserve EK5WwpsRuWPCAhV4Rd4s5SRuE6Gnbc8SA94oUjZbHfVb: 50% SOL, 50% SSR, launched successfully 16:04-16:05 the same day), saw SOL leave and WSOL arrive in the first wallet transaction despite USDC being named the deposit asset, then the buy failed on-chain with a custom program error and no CHARLI Reserve Tokens arrived. Instructed: trace everything from first principles, no assumptions from the UI message, read-only Mainnet inspection only, no push/deploy/on-chain mutation. (Note: the task named branch fable-functional-port, which is a stale ancestor of main predating the entire Mainnet integration -- the failing code lives on main, where this fix was made.)",
  "rationale": "Evidence chain: failed signatures 32SbHWM8..., 5eePAsTV..., Z6v6BGFN... (16:07:09/16:07:54/16:08:12, all InstructionError[5,{Custom:3002}]); decoded logs name manager_fee_recipients + AccountDiscriminatorMismatch; the wire's 13 accounts match the regenerated IDL exactly; programdata slot 440248996 proves no upgrade since 2026-08-19, i.e. the deployed struct is c2b84ba's (read directly from git); iterative read-only simulateTransaction runs discriminated between candidate layouts and the final corrected build simulates to err:null. Asset-flow reconstruction (all signatures decoded): 16:04:30 create+register (4Qcmj...), 16:05:22 wrap 0.101896 SOL (4cpCq...), 16:05:37 seed -- deposited 0.101896 wSOL + 16,187.96 SSR, minted 19.80 CHARLI to the creator (2t8Fj...), 16:07:01 the buy's wrap of 0.05151 SOL (4faia...), then the three failed mints. Where every asset ended: 0.051510414 wSOL in the buyer's own wSOL ATA (53956dY7...), SSR untouched at 154,278.34 (63UUcPv7...), USDC untouched at 166.09, CHARLI balance still the seed's 19.8 (9m2etkvp...) -- NOTHING stranded in any vault/PDA/intermediate account (the mint never executed; each failure was atomic); only ~0.0004 SOL of fees burned across the three failed attempts. The old recovery message's claims were factually TRUE for this state (assets in the buyer's wallet; retry counts them) but were asserted from client state, not verified -- the new report verifies on-chain before claiming. No CHARLI arrived because the mint instruction never executed successfully -- the failure was in account validation, before any token movement.",
  "alternativesConsidered": [
    "Deploy a Mainnet program upgrade to match the current source instead of pointing the client at the deployed shape -- rejected and out of scope: explicitly forbidden this pass, materially riskier, and unnecessary (the deployed binary fully supports the intended flow, proven by simulation).",
    "Regenerate the whole SDK IDL from c2b84ba -- rejected: DEC-0133's client-side builders for the future fee-settlement instructions (redeemFeeVaultShares etc.) type-check against the regenerated IDL; only the two DRIFTED instructions used against the deployed binary (mint_reserve_tokens_in_kind, accrue_fees -- found by a full instruction-by-instruction diff; redeem_reserve_tokens_in_kind and seed_reserve did NOT drift) needed restoring. The four SDK-only instructions cannot be called against the deployed binary regardless.",
    "Keep funding the wrapped-SOL buy leg from native SOL and just disclose it in the UI -- rejected: the Creator's requirement is explicit (a USDC-denominated buy debits USDC plus only genuine network/rent SOL), and DEC-0151's documented invariant already promised exactly that."
  ],
  "impact": "852/852 offline tests passing (23 new/updated: tests/phase_mainnet_buy_mint_shape.ts pins both deployed account lists position-by-position and the built two-asset SOL+SSR wire shape, covers USDC-funded wSOL handling, retry-after-partial, duplicate-purchase prevention, insufficient USDC/SOL, double-mint guard incl. rounding tolerance, post-failure reporting, and CHARLI-real delivery math; tests/phase_protocol_fee_auto_deposit.ts's two 2026-08-21 assertions that had enshrined the never-deployed shape now assert the DEPLOYED shape and say they may only change with a real Mainnet upgrade). tsc -b and npm run build clean. scripts/verify_mint_account_shape_mainnet.cjs kept as a permanent read-only verifier (expected output: err null). NOT pushed, NOT deployed, no on-chain state modified -- REMAINING BLOCKER: production still runs the broken IDL; no Mainnet buy of ALPHA/BETA/CHARLI can succeed until this commit is deployed.",
  "affectedAreas": ["packages/sdk/idl/ssr_protocol.json", "packages/sdk/idl/ssr_protocol.ts", "src/merge/lib/multiAssetBuyPlan.ts", "src/merge/lib/multiAssetBuyClient.ts", "src/merge/lib/jupiterSwapClient.ts", "src/merge/pages/DTRDetail.tsx", "api/mainnet/jupiter-swap.ts", "scripts/verify_mint_account_shape_mainnet.cjs", "tests/phase_mainnet_buy_mint_shape.ts", "tests/phase_protocol_fee_auto_deposit.ts", "docs/project/PROJECT_STATUS.md"],
  "supersedes": "DEC-0133 (partially: its IDL regeneration, until a real Mainnet program upgrade ships); DEC-0140/DEC-0151 (partially: the buy's wrapped-SOL funding path)",
  "supersededBy": null,
  "evidence": [
    "Failed buy signatures 2026-08-25: 32SbHWM8BWiRetV8Lbn4R12yHNSURFfmnfhw75chfRzsLHR5ogQmZZ9xjMZWD9mYRdu57w3BLN1HA5qdhzehgqZG, 5eePAsTVBTXGCEfFqtZPwVVTNsbY95Gz99PsUQFhs6oZZsEsRrDfTNDQQRN8uxmv9vokGTsxjSidVPdjQ6xamsNp, Z6v6BGFN8caDXid5yUKCFSapUPc9CJXZpqXNG4nFifbWTtkawFWYmc3VTAaN3Le22HEpAGj3My3YVT98P7rtUo3 -- all InstructionError[5,{Custom:3002}], logs: 'AnchorError caused by account: manager_fee_recipients. Error Code: AccountDiscriminatorMismatch'.",
    "Program 8hTW7fHwn8t8hcgTVeyAhHMiCTHGUP3783NWUTBBFwH9's programdata (2YF7aofgPbNj96pJJh1uzLtCk4H2fUq1FVLxFf9TAwow): last deployed slot 440248996 = 2026-08-19T10:34:43Z -- no upgrade since; deployed struct read from git commit c2b84ba's mint_reserve_tokens_in_kind.rs.",
    "packages/sdk/idl mtime 2026-08-21 10:08/10:00 (the DEC-0133 pass) -- the regeneration that introduced the drift; full instruction-by-instruction diff vs target/idl (the deployed-era IDL) found exactly two drifted instructions plus four SDK-only ones.",
    "Read-only Mainnet simulateTransaction of the corrected CHARLI mint (unsigned, sigVerify off): err null, MintReserveTokensInKind executes to success incl. inner token CPIs -- the definitive proof, obtained without spending anything.",
    "Asset-flow decode of every buy-window signature (16:04:30-16:08:12) and current balances: wSOL 0.051510414 (buyer ATA 53956dY7...), SSR 154,278.336552, USDC 166.088659, CHARLI 19.8, native SOL 1.2568 -- nothing in any vault/PDA/intermediate account.",
    "852/852 offline tests passing."
  ]
}
```
...
```json
{
  ...
  "date": "2026-08-26",
  "status": "confirmed-implemented",
  "decision": "Tightened the Mainnet buy's funding accounting from whole-wallet to PURCHASE-SCOPED (completing DEC-0154, before its first deployment), and deployed the combined fix to production. (1) A leg now counts ONLY what THIS purchase's own confirmed swaps acquired -- reconciled from each recorded swap signature's real on-chain token delta (getParsedTransaction pre/post token balances, computeOwnerTokenDeltaRaw), cumulatively persisted per leg (PersistedAssetFunding.acquiredRaw), and capped at what the wallet still actually holds. Assets the wallet already held for other reasons are NEVER silently consumed in place of the quoted USDC. (2) Every previously-recorded swap signature is reconciled BEFORE planning (confirmed -> its measured output is counted, never repeated; definitively failed/expired -> its leg resets cleanly; confirmed-but-unfetchable -> the purchase stops with nothing submitted rather than guessing either way). (3) Pending-purchase persistence became a map keyed wallet+reserve (ssr_pending_buys_v2) so concurrent purchases of different Reserves can no longer overwrite each other's double-mint baselines and acquired-asset records (the v1 single-slot shape could). (4) The post-failure report now names the exact stage that failed in plain language (failedStage) and shows, per leg, what this purchase acquired vs. what the wallet merely holds; its retry summary states explicitly that unrelated wallet assets are never used in place of USDC.",
  "context": "Creator retried the CHARLI buy twice on 2026-08-26 (09:05:36 and 09:05:58 UTC) against production, which still ran the pre-DEC-0154 build -- both failed identically: InstructionError[5,{Custom:3002}] (Anchor AccountDiscriminatorMismatch on manager_fee_recipients, ix 5 = the first ssr_protocol instruction after 2 compute-budget + 3 idempotent ATA creations), signatures 3qq3D1nBPnM2k3LFmNPwYjTL9enTerAcPdA9RvWubgsVN9LhgomKqZJiTzWaRLnt33BjkNHLuCuiJKjmjF6Qq1R4 and 5LxJ5oif7Ufo4132Mhamm7LVNf6gxaaKBL6xbTi4vfTphpMsuvDuo3tNEFqbBboQAJUXgv9SUDwDbzDi4CuqDnPW (the one in the Creator's screenshot). Each burned only the 0.000085 SOL fee; token balances unchanged (verified from both transactions' pre/post balances). Creator then mandated the full invariant explicitly: the user supplies USDC only; existing unrelated wallet assets must not silently replace the quoted USDC investment; fee SOL stays separate; retry resumes only genuine deficits and never double-spends.",
  "rationale": "DEC-0154's planBuyFunding counted the wallet's WHOLE balance toward each leg. For the live wallet that meant the SSR leg would be satisfied by 154,278 pre-existing SSR and the wSOL leg by the 0.05151 wSOL wrongly wrapped from native SOL -- a retry would have delivered the ~$10 CHARLI purchase while charging almost none of the quoted USDC, converting unrelated holdings into the Reserve deposit instead. Purchase-scoped accounting preserves both halves of the requirement simultaneously: recorded confirmed swaps are never repeated (reconciled by signature + measured by that transaction's own token delta -- authoritative even across refresh/reconnect), while unrelated holdings never substitute for USDC. Since NO USDC was ever debited for this purchase (verified across every signature in the buy window), the corrected retry charges the quoted ~$10 USDC once -- a first charge, not a second -- and the stranded 0.05151 wSOL (the Creator's own wrongly-wrapped native SOL) is deliberately NOT consumed: it stays in the Creator's wSOL ATA, disclosed in the report, unwrappable back to native SOL at any time.",
  "alternativesConsidered": [
    "Keep DEC-0154's whole-wallet counting (retry would mint CHARLI with ~zero further USDC by consuming held SSR + the stranded wSOL) -- rejected: violates the Creator's explicit invariant that unrelated wallet assets never replace the quoted USDC investment, and perpetuates exactly the observed '-SOL +WSOL while the UI said USDC' confusion.",
    "Automatically unwrap the stranded 0.05151 wSOL back to native SOL during the next buy -- rejected: moves Creator funds beyond the purchase itself without explicit approval; the report disclosing it (and it remaining spendable/unwrappable by the owner) is sufficient.",
    "Server-side persistence of purchase state instead of localStorage -- rejected for now: the state is per-wallet client state reconciled against the chain (the chain remains the authority); localStorage already survives the refresh/reconnect cases reported, without adding an authenticated server store."
  ],
  "impact": "864/864 offline tests passing (33 in tests/phase_mainnet_buy_mint_shape.ts: 12 new/updated for purchase-scoped semantics incl. the literal live wallet state -- 154,278 unrelated SSR + 51,510,414 raw stranded wSOL still swaps USDC for BOTH legs in full; moved-out-mid-purchase capping; zero-swap retry; computeOwnerTokenDeltaRaw incl. fresh-ATA and foreign-owner cases; the complete decoded live error incl. instruction index 5 and the exact serialized shape; ssr_pending_buys_v2 map persistence: refresh round-trip, two-Reserve coexistence, wallet/reserve isolation, corrupt-JSON tolerance). tsc -b, oxlint, npm run build clean. Read-only Mainnet simulation of the corrected CHARLI mint RE-VERIFIED against current chain state this session: err null, full success. Deployed to production 2026-08-26 (deployment ID + live verification recorded in PROJECT_STATUS.md's 2026-08-26 entry). RETRY SAFETY, proven: with this deployed, clicking Buy again charges the quoted USDC once (never twice -- acquired legs are recorded and reconciled), never consumes the pre-existing SSR/wSOL, and the double-mint guard blocks any second mint.",
  "affectedAreas": ["src/merge/lib/multiAssetBuyPlan.ts", "src/merge/lib/multiAssetBuyClient.ts", "src/merge/lib/launchFunding.ts", "src/merge/pages/DTRDetail.tsx", "tests/phase_mainnet_buy_mint_shape.ts", "docs/project/PROJECT_STATUS.md"],
  "supersedes": "DEC-0154 (partially: its whole-wallet already-held counting and single-slot ssr_pending_buy_v1 persistence)",
  "supersededBy": null,
  "evidence": [
    "2026-08-26 failed retries fetched from Mainnet: 09:05:36Z 3qq3D1nB... and 09:05:58Z 5LxJ5oif7Ufo4132Mhamm7LVNf6gxaaKBL6xbTi4vfTphpMsuvDuo3tNEFqbBboQAJUXgv9SUDwDbzDi4CuqDnPW, both InstructionError[5,{Custom:3002}], logs naming 'AnchorError caused by account: manager_fee_recipients. Error Code: AccountDiscriminatorMismatch. Error Number: 3002', fee 85000 lamports each, token balances byte-identical pre/post.",
    "Wallet state re-verified 2026-08-26: wSOL 0.051510414, SSR 154,278.336552, USDC 166.088659, CHARLI 19.8, native SOL 1.25658743 -- unchanged from DEC-0154's record except the two retries' fees; ZERO USDC has ever been debited for this purchase.",
    "Read-only Mainnet simulateTransaction of the corrected mint, re-run this session: err null, MintReserveTokensInKind success incl. inner token CPIs.",
    "864/864 offline tests passing."
  ]
}
```
...
```json
{
  ...
  "date": "2026-08-26",
  "status": "confirmed-implemented",
  "decision": "(1) ROOT CAUSE FIX for the post-DEC-0155 buy failure: EVERY Jupiter swap the app builds now sets wrapAndUnwrapSol:false (api/mainnet/jupiter-swap.ts, universal -- previously only a wrapped-SOL-output leg did). (2) ONE-TRANSACTION BUY: the entire purchase -- every Jupiter swap (via the endpoint's new mode:'instructions' returning raw instructions + address-lookup-table addresses), any recovered-SOL re-wrap, ATA creations, and the final mint_reserve_tokens_in_kind -- is now composed into a SINGLE v0 transaction the wallet approves ONCE (singleTxBuy.ts), read-only-simulated BEFORE the signature is requested, atomic on-chain (X USDC out -> Y Reserve Tokens in, or nothing moves but the fee). Falls back to the sequential state-machine flow only on SingleTxTooLargeError (many-asset Reserves), and that fallback now re-reads EVERY leg balance after all funding transactions before the mint. (3) RECOVERED-SOL RE-WRAP: a wSOL leg whose recorded purchase-acquired amount exceeds the still-wrapped balance plans a wrap-recovered-sol action -- re-wrapping exactly the recorded remainder from native SOL (capped at the deficit), never fee SOL, never unrelated holdings, never a second USDC spend. (4) SPL Token inner-CPI errors (0,1,2,3,4,6,9,12,14,17) decoded honestly in describeOnChainError with a most-likely hedge. (5) Toast UX: error toasts render fully in the bottom-right (min-w-0 + overflow-wrap:anywhere fix the offscreen-overflow of unbreakable signatures/JSON; long content scrolls inside the toast), the text is selectable (pointer-down stopPropagation vs Radix swipe), and a copy control copies the full title+description.",
  "context": "First retry after DEC-0155 deployed (2026-08-26 10:21 UTC): BOTH swaps succeeded exactly as designed -- 10:21:22 USDC->wSOL (+50,654,504 raw wSOL for 4.885722 USDC, sig 3thjKXxb36RG9kzr548wAPWaJYraSXLwrFBsBWC4yjo9ugKUPCzkF2bVRn77DJcjGgiBt1Hkx1TQGCGWH73vXg4c) and 10:21:31 USDC->SSR (+7,966,690,976 raw SSR for 5.314278 USDC, sig n6JMY9xFfG7Vd1grgq93uxPk81XgrCwrxQrXHfveh7zXgMdQ4fftSWX54dp6P5KLggJtBTzizt8MKvB1aFR6VNB) -- but the SSR swap's route hopped through SOL and, with Jupiter's default wrapAndUnwrapSol:true, its cleanup CLOSED the buyer's wSOL ATA (closeAccount 53956dY7... -> owner, decoded from the transaction), sweeping the ENTIRE 0.102164918 wSOL balance (the just-bought leg AND the stranded 0.051510414) into native SOL (+0.104176345 incl. rent refund). The mint 8s later (10:21:39, sig 4Mgj2LxT1PdQaA2FAaKaMZv34pgrvSABbKWcALT5LJQ38YFS7Fppcc8gPJbaVvXKw2K4HDkvURDeZKxqWMUTMNE4) failed InstructionError[5,{Custom:1}] -- the logs' own 'Program log: Error: insufficient funds' from the wSOL deposit transfer CPI. Creator also specified: error toasts were clipped offscreen and uncopyable, and the purchase must be ONE Phantom transaction.",
  "rationale": "Atomicity eliminates the whole class: inside one transaction there is no window for a later transaction to destroy an earlier leg's funding, no partial states, no multi-approval UX. The universal wrapAndUnwrapSol:false remains necessary even within the single transaction (a cleanup instruction would close the wSOL ATA before the mint instruction runs) and also fixes the same latent sweep in the sequential fallback and the launch flow. The re-wrap is the honest resume for the CURRENT stranded state: the purchase's 10.2 USDC was fully spent and its wSOL output exists in the wallet as native SOL -- re-wrapping the recorded amount restores the purchase's own asset instead of charging ~$5 USDC again. PROVEN read-only against live Mainnet: the exact composed retry (re-wrap 49,616,739 lamports + ATA creations + mint, 8 instructions, 950 bytes) simulates err:null, minting ~9.59 net CHARLI.",
  "alternativesConsidered": [
    "Custody-style server-side execution (user sends USDC once, protocol performs swaps/mint) -- rejected: requires the server to hold user funds mid-flight, violating the never-custodies-funds model; client-side atomic composition achieves the same one-approval UX trustlessly.",
    "useSharedAccounts:true to keep routes out of user ATAs -- not chosen as the primary fix: not supported on all routes, while wrapAndUnwrapSol:false deterministically removes the destructive cleanup.",
    "Counting the swept wSOL's native-SOL equivalent implicitly (skip the wrap, wrap at mint time) -- rejected: the deposit instruction needs SPL wrapped SOL; an explicit, recorded, capped re-wrap action keeps the accounting inspectable and testable.",
    "An on-chain program change to make the mint wrap SOL itself -- out of scope: program upgrades are explicitly gated, and the client-side fix is sufficient (proven by simulation)."
  ],
  "impact": "878/878 offline tests passing (14 new in tests/phase_single_tx_buy.ts: the live stranded state plans wrap+zero-USDC, wrap capped at recorded acquisition, non-wSOL never wraps, feasibility covers wrap lamports, retry-summary wording, Jupiter instruction deserialization, wrap instruction exactness, composition ordering + compute-budget dedup + mint-last + no-cleanup, size guard incl. the RangeError overrun path, the complete decoded Custom(1), universal wrapAndUnwrapSol:false on a plain USDC->SSR build, instructions mode round-trip, default mode unchanged; 1 DEC-0155 test updated: a moved-out wSOL portion now re-wraps instead of re-swapping). tsc -b, oxlint, npm run build clean. scripts/verify_single_tx_buy_mainnet.cjs kept as the permanent read-only composed-retry verifier (expected: err null). Deployed to production 2026-08-26 (ID + live verification in PROJECT_STATUS.md).",
  "affectedAreas": ["api/mainnet/jupiter-swap.ts", "src/merge/lib/singleTxBuy.ts", "src/merge/lib/multiAssetBuyPlan.ts", "src/merge/lib/multiAssetBuyClient.ts", "src/merge/lib/jupiterSwapClient.ts", "src/merge/pages/DTRDetail.tsx", "src/merge/components/ui/toast.tsx", "src/merge/components/ui/toaster.tsx", "packages/sdk/src/errors.ts", "scripts/verify_single_tx_buy_mainnet.cjs", "tests/phase_single_tx_buy.ts", "tests/phase_mainnet_buy_mint_shape.ts", "docs/project/PROJECT_STATUS.md"],
  "supersedes": "DEC-0154/DEC-0155 (partially: the sequential multi-transaction flow is now the fallback, not the primary path; receiveWrappedSol is subsumed by the universal wrapAndUnwrapSol:false)",
  "supersededBy": null,
  "evidence": [
    "Swap 3thjKXxb... (10:21:22Z): buyer wSOL 51,510,414 -> 102,164,918 raw; USDC 166.088659 -> 161.202937.",
    "Swap n6JMY9xF... (10:21:31Z): +7,966,690,976 raw SSR; closeAccount of the buyer's wSOL ATA 53956dY7... to the buyer; buyer native SOL +0.104176345; wSOL ATA confirmed nonexistent afterward.",
    "Mint 4Mgj2LxT... (10:21:39Z): InstructionError[5,{Custom:1}]; logs: 'Program log: Error: insufficient funds' inside MintReserveTokensInKind's first token CPI; pre-token balances show NO wSOL entry.",
    "Wallet after: USDC 155.888659 (the full ~10.2 USDC purchase debited), SSR 162,245.027528, wSOL ATA closed, native SOL 1.360668158, CHARLI still 19.8.",
    "Read-only Mainnet simulation of the composed single-transaction retry (8 instructions, 950 bytes): err null, MintReserveTokensInKind success.",
    "878/878 offline tests passing."
  ]
}
```
...
```json
{
  ...
  "date": "2026-08-26",
  "status": "confirmed-implemented",
  "decision": "Full-function pass bringing the live Mainnet surface up to the DevNet-approved spec, four fixes: (1) SELL for every Mainnet composition: new SDK buildDirectMultiAssetRedeemInstructions (deployed account shape, computeRedemptionEntitlements' exact floor math) + multiAssetSellClient.ts -- Reserve Tokens out -> USDC in, redeem + every non-USDC leg sold into USDC via Jupiter, composed into ONE wallet-approved atomic transaction where it fits (read-only-simulated before signature) with a persisted sequential fallback (ssr_pending_sells_v1: redeem/swap signatures reconciled before any resubmission; a landed redeem is NEVER re-burned, a landed swap never repeated); DTRDetail routes pure-USDC Reserves to the direct USDC redeem and everything else to the USDC-settled path -- the 'Sell Not Yet Supported' gate is gone. The Jupiter proxy gained the sell direction (inputMint -> USDC only; exactly one side of every swap is always USDC). (2) LIVE 24h/7d PERFORMANCE: new pure refreshPriceSeries appends the freshly-computed NAV to the price history on EVERY sync (5-min throttle, immediate on a >=0.1% move, 0-price placeholders dropped) and derives change24h/change7d from it -- wired into both mergeOnChainIntoDTR and mergeDiscoveredReserves (they constructed 0 and never recomputed; history only grew on the user's own trades). (3) HONEST COST BASIS: syncRealHolding no longer overwrites avgPurchasePrice with the current nav (the DEC-0149 root cause of structurally-$0 Unrealized P&L); a confirmed Buy moves the basis via the new pure weightedAvgCostBasis (actual USDC paid / tokens received); a Sell leaves it unchanged. (4) DISCOVERY/STATS CANDIDATES FROM THE CHAIN: new SDK enumerateReserveAssetMintsOnChain (one gPA discriminator scan over ReserveAsset accounts -- exactly the fix discovery.ts's own header prescribed once a paid RPC allowed it) is unioned into api/ledger/known-asset-mints (which every fresh client's discovery uses) and api/mainnet/landing-stats' candidates; landing-stats also now prices volume with live Jupiter prices for every asset (was USDC-only, so every non-USDC leg counted $0).",
  "context": "Creator, after the first successful one-transaction buy: gains/7D performance not updating with real asset price variations; 'sell not yet supported' contrary to spec ('all functions that we checked and approved on devnet were supposed to be triggered here and rolled out from the get go'); ordered a thorough once-over of all functions. Audit found additionally: landing-stats counted ONE reserve (9KkRx62F..., pure USDC) with $0 volume and CHARLI absent (holder count/24h volume showed 'unavailable' on its page), and /api/ledger/known-asset-mints returned an EMPTY list -- the Mainnet ledger has ingested zero events (api/ledger/ingest-cron.ts still points at DevNet via LEDGER_CLUSTER), meaning a fresh browser could not even resolve a non-USDC Reserve's assets.",
  "rationale": "Sell mirrors the proven buy architecture exactly (atomic composition, wrapAndUnwrapSol:false everywhere, persisted-signature reconciliation, delivery verified from the seller's real USDC balance). The multi-asset redeem was PROVEN by read-only Mainnet simulation against live CHARLI (err null, both legs paid; also confirming the buy landed: supply 29.495496, vaults 0.15128 wSOL + 24,032.79 SSR). Client-side price history is honest observed data (persisted per browser, growing on every sync/discovery across sessions) -- a server-side NAV snapshot ledger remains future work, as does pointing the ledger ingest cron at Mainnet. gPA enumeration is chain-authoritative and cheap (24 in-use mints enumerated live), removing the empty-ledger single point of failure while keeping the ledger as a secondary source.",
  "alternativesConsidered": [
    "Repair the Mainnet ledger ingestion first and keep candidates ledger-derived -- rejected as the primary fix: ingestion is a daily-cron indexer with its own backfill work, while the chain scan is authoritative, immediate, and simpler; ingestion remains listed as remaining work for the activity/volume event history.",
    "Server-side NAV snapshots for 24h/7d performance -- deferred: client-side observed history (persisted, cross-session) delivers live-updating figures now without new storage; noted as the durable follow-up.",
    "Keeping sell in-kind only (deliver the basket assets, let the user sell manually) -- rejected: the approved product spec settles in USDC, exactly like DevNet's devUSDC-settled sell."
  ],
  "impact": "892/892 offline tests passing (13 new in tests/phase_mainnet_sell_and_perf.ts: the literal +0.00% symptom reconstructed and fixed, sync throttling, placeholder-point hygiene, rolling-window bases, cost-basis weighted math incl. the live DEC-0149 numbers, redeem builder shape/sentinel/entitlements + single-asset refusal, pending-sale persistence round-trip/isolation/corruption, sell-direction proxy validation incl. never-forwarded bad requests + wrapAndUnwrapSol:false; 1 discovery-merge test updated to the growing-history semantics; known-asset-mints' both-sources-failed case still 503s, caught and fixed by the existing sanitized-failure test). tsc -b, oxlint, npm run build clean. scripts/verify_multi_asset_redeem_mainnet.cjs kept as the permanent read-only redeem verifier (expected: err null). Deployed to production 2026-08-26 (ID + live verification in PROJECT_STATUS.md). AUDIT NOTES: ManageDTR (delegates, targets, fee routing, rebalance, close/wind-down) is fully cluster-aware and permission-gated with no Mainnet feature gates -- wired since DEC-0113/0114, not exercised live this session (manager state changes require explicit Creator action); REMAINING KNOWN WORK: Mainnet ledger ingestion (LEDGER_CLUSTER env + backfill) for the durable event history behind volume/activity, and server-side NAV snapshots for performance figures that predate a browser's first visit.",
  "affectedAreas": ["packages/sdk/src/directInstructions.ts", "packages/sdk/src/discovery.ts", "src/merge/lib/multiAssetSellClient.ts", "src/merge/lib/jupiterSwapClient.ts", "src/merge/lib/calculations.ts", "src/merge/lib/onChainReserve.ts", "src/merge/store/useAppStore.ts", "src/merge/pages/DTRDetail.tsx", "api/mainnet/jupiter-swap.ts", "api/mainnet/landing-stats.ts", "api/ledger/known-asset-mints.ts", "scripts/verify_multi_asset_redeem_mainnet.cjs", "tests/phase_mainnet_sell_and_perf.ts", "tests/phase_data_integrity.ts", "docs/project/PROJECT_STATUS.md"],
  "supersedes": "DEC-0140/DEC-0154 (partially: the documented multi-asset-sell gap); DEC-0149 (its deferred pricing/P&L bugs, now fixed)",
  "supersededBy": null,
  "evidence": [
    "Live landing-stats before the fix: {holders:2, vol24h:0, reservesCounted:1, perReserve:{9KkRx62F...}} -- CHARLI absent; /api/ledger/known-asset-mints: {mints:[]}.",
    "enumerateReserveAssetMintsOnChain against live Mainnet: 24 real in-use mints (SOL, USDC, SSR, and every launched Reserve's assets).",
    "Read-only Mainnet simulation of buildDirectMultiAssetRedeemInstructions on live CHARLI (1 RT): err null, entitlements 5,128,777 lamports wSOL + 814,795,175 raw SSR paid to the redeemer's own ATAs.",
    "892/892 offline tests passing."
  ]
}
```
