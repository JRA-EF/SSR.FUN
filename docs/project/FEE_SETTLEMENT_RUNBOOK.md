...
## 7. As executed on Mainnet (2026-09-07/08, DEC-0186)

- **Keeper wallet:** `AuaJRdbRZhhfdqpyjN8uNYSnggRwLY8kzHVjngkGPhsF` (fresh keypair; secret held only in the Vercel Production env `SSR_FEE_SETTLEMENT_KEEPER_SECRET`, Sensitive; funded 0.05 SOL from the developer deploy wallet, tx `4qYiHisx...3Zdc`). Top up when `keeperSolBalance` in the dry-run drops below ~0.01.
- **Schedule:** `vercel.json` crons `15 * * * *` (hourly) -> `/api/mainnet/fee-settlement-cron`, allowlisted in `middleware.ts` `CRON_PATHS`. Each run: Job A settlement (only once the on-chain keeper matches), then Job B `accrue_fees` for every Reserve >= 7 days since its last accrual, within a 50s budget; leftovers are picked up next hour.
- **Step 1 (on-chain keeper) is a Protocol-Admin signature, not a Squads action:** open `https://ssr.fun/internal/set-keeper` (site password), connect the Creator (`CgHFxD4...`) or Boss (`PSpQ...`) wallet, click *Sign & send*. The page simulates first and reads `SettlementKeeperConfig` back; the cron picks it up on its next hourly run.
- **Verify any time:** `GET /api/mainnet/fee-settlement-cron?dryRun=true` (no secret) -- `settlementEnabled`, `settlementBlockedBy`, candidates with `feeSettlement` balances, `accrualsDue`.
- **Deviation from step 4:** the first automated settlement run will process the current candidates directly (fee vaults holding cents' worth of Reserve Tokens) and be verified on-chain afterwards -- see DEC-0186.
- **Observed cost (first scheduled run, 2026-09-08 00:15 UTC):** each Reserve's FIRST `accrue_fees` costs ~0.0044 SOL (one-time rent for its FeeSettlement, fee-vault ATA and TvlAccrual accounts) + 0.000005 fee; 12 landed, the 13th failed with system error 0x1 (keeper out of SOL at 0.05). Keeper topped up +0.3 SOL. Budget ~0.002 SOL per (Reserve x asset) staging ATA the first time settlement touches it; steady-state cost is transaction fees only.
