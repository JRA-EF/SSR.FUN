...
**(2026-08-17, DEC-0105 clarification):** the "Upgrade authority pubkey" above is Creator (Claude User)'s wallet -- "Creator (Claude User)" meaning the individual who currently controls and develops the protocol, not a "Reserve creator" (an SSR.fun user who launches a Reserve). Creator (Claude User) explicitly confirmed keeping sole control of this key for the current early stage; a separate treasury multisig (Creator + the boss) covers company treasury assets only and does not receive this authority. See `docs/project/PROJECT_STATUS.md`'s "Authority & Treasury Structure" section and `docs/project/DECISION_LOG.md` DEC-0105.
...
- **(2026-08-17, DEC-0105 clarification):** this item (and `public/road-to-mainnet.html`'s matching MS-02/PM-08 checklist entries) remains the documented eventual target and is NOT fulfilled or scheduled by DEC-0105. For the current early stage, Creator (Claude User) explicitly confirmed retaining this upgrade authority (plus protocol admin and deployment authority) solely on their own wallet -- the boss receives no protocol authority and no access to Creator (Claude User)'s keys. A separate treasury multisig (Creator (Claude User) + the boss), being formed now, is scoped to company treasury assets/transactions only; it must not automatically become upgrade authority, protocol admin, deployment authority, or controller of Reserve Asset vaults (which remain program-controlled PDAs, unchanged). No migration date or TVL threshold is set; any future transfer of this authority requires a separate, later, explicit decision. See `docs/project/DECISION_LOG.md` DEC-0105 and `docs/project/PROJECT_STATUS.md`'s "Authority & Treasury Structure" section.
...
- **(2026-08-17, DEC-0106):** every locked/at-risk/recoverable-SOL situation from this process (orphaned buffers, abandoned accounts, program rent-exemption) is tracked permanently in `docs/project/FUNDS_LEDGER.md`, not just noted here in passing.
