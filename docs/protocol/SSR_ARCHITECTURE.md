...
- **Root Reserve Manager**: full control over their own Reserve's configuration (metadata, targets, fees, co-managers, pause) and exclusive control over irreversible actions (authority transfer, unrestricted-co-manager grant/revoke). Cannot withdraw Reserve Assets outside a valid redemption or explicitly-validated rebalance-execution flow -- there is no operator-withdrawal instruction anywhere in the instruction set.
- **Co-Managers**: only the specific permission flags they've been granted; enforced by an Anchor `constraint` check against the `Delegate` PDA's permission bitmask on every gated instruction, not by client-side trust.
