...
| Field | Type | Purpose |
|---|---|---|
| `schema_version` | `u8` | account layout version, for future migration (bumped 1 -> 2 for `admin_2`, Mainnet authority-model pass) |
| `authority` | `Pubkey` | first Protocol Admin wallet |
| `admin_2` | `Pubkey` | second Protocol Admin wallet -- both wallets act independently with equal authority (`ProtocolConfig::is_admin`); there is deliberately no multisig/threshold layer at this level |
| `paused` | `bool` | global emergency pause -- blocks creation/mint/rebalance program-wide; **never blocks redemption** on any existing Reserve (see SECURITY_INVARIANTS.md); set via `set_protocol_paused` by either admin |
...
Narrow by design: this account cannot name itself as a vault owner, cannot appear in any `ReserveVault`'s owner chain, and no instruction lets either `authority` or `admin_2` move tokens out of any Reserve Vault. Compromise of either admin wallet can pause creation of new Reserves and change defaults for future Reserves -- neither can touch a single token in any already-created Reserve.
...
**Mainnet target and code-support finding (2026-08-18, DEC-0112):** the final temporary Mainnet target is Creator's Protocol Admin wallet (`CgHFxD4XHZzmSGEomnMXipGo75ejqhVd5aNY4GHg4Rw8`) as `authority`, plus a second, independent Boss Protocol Admin wallet (`PSpQGPvw7tZedKJvN21dJkh3vdDQeXkwA5n9DKBRZw5`) intended to hold protocol-admin authority on equal footing (never program upgrade/closure authority). **`authority` above is a single `Pubkey` field, not a `Vec<Pubkey>` or a fixed 2-element array -- the deployed and repository code, as of this note, structurally cannot recognize a second independent admin wallet.** `update_protocol_config` (`instructions/update_protocol_config.rs`) -- the only admin-gated instruction that exists -- checks it via one Anchor `has_one = authority @ SsrError::NotProtocolAuthority` constraint. Genuinely supporting two independent Protocol Admins requires an on-chain program upgrade: widening `ProtocolConfig` (a new field, e.g. a second `Pubkey` or a small fixed array) and updating the authority check in every admin-gated instruction to accept either. Because no Mainnet `ProtocolConfig` account exists yet, this is a pre-genesis Rust/rebuild/deploy change, not a live-state migration. Separately confirmed in the same investigation: no instruction anywhere writes `ProtocolConfig.paused` after `initialize_protocol` sets it to `false` once -- there is currently no way to pause or unpause the protocol at all, for any admin (see `docs/protocol/SSR_ARCHITECTURE.md` section 7's annotated correction). The company Squads Treasury vault (`3CBpVMPDQD75b5bXgDunkpVJ3EeQWcwU9DCSLsTjWQL5`, verified on-chain as the genuine Squads vault-index-0 PDA of a real 2-of-3 Squads multisig) is confirmed PDA/ATA-compatible as `default_protocol_fee_destination` -- `collect_protocol_fee.rs`/`collect_fees.rs` never require that account to sign. None of this has been deployed, configured, or transferred; see `docs/project/DECISION_LOG.md` DEC-0112 for the full investigation and `docs/project/PROJECT_STATUS.md`'s "Authority & Treasury Structure (Mainnet target)" section for the full matrix.
