...
// CommonJS-native session/auth helper used ONLY by api/kpis/*.ts.
//
// This is a deliberate DUPLICATE of lib/dashboard/session.ts +
// lib/road-to-mainnet/auth.ts's logic, not a reuse of either -- both of
// those live in directories governed by the root tsconfig.node.json
// project (ESM/nodenext, no CommonJS-scoping package.json), while
// api/kpis/ is CommonJS (api/kpis/package.json, mirroring api/devnet's
// pattern). A CommonJS module can only `require()` another CommonJS
// module synchronously -- requiring a genuine ESM module this way is a
// hard Node.js error (ERR_REQUIRE_ESM) that crashes at IMPORT time,
// before the handler function even runs, invisible to any try/catch
// inside the handler. This was traced live in production
// (docs/project/DECISION_LOG.md's entry for this pass): kpis.ts and
// kpis-refresh.ts (which imported lib/road-to-mainnet/auth.ts) returned a
// raw platform 500 with no readable error; kpis-backfill-cron.ts (which
// never imported anything from lib/dashboard/ or lib/road-to-mainnet/)
// worked every time. Keep it that way: nothing in api/kpis/ may import
// from lib/dashboard/ or lib/road-to-mainnet/ ever again.
//
