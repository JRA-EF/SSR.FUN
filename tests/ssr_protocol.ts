...
describe("ssr_protocol", () => {
  ...
  describe("cross-Reserve isolation", () => {
    it("rejects an instruction that supplies a vault belonging to a different Reserve", async () => {
      // Create a second Reserve, register one asset on it, then attempt to
      // call mint_reserve_tokens_in_kind on the FIRST Reserve while passing
      // the SECOND Reserve's vault as a remaining account -- this must fail
      // the seeds constraint in load_asset_legs (InvalidReserveVault),
      // proving cross-Reserve vault substitution is structurally rejected,
      // not just discouraged. Left as a TODO stub pending Gate 6 SDK helpers
      // to reduce the boilerplate of standing up a second full Reserve here.
    });
  });
});
