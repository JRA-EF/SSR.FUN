...
import type { DTR, DTRAsset, FeeConfig, PricePoint } from "./types";
...
// Anchored to real load time (not a fixed past date) so the seeded walk always ends
// at "now" -- a live trade's appended point then continues the line without a gap.
const NOW = Date.now();
const MINUTE = 60_000;
...
/**
 * Generates one continuous, deterministic random walk ending exactly at `endPrice`,
 * denser near "now" so the shorter chart timeframes have real data to aggregate:
 * daily resolution out to a year ago, hourly for the last week, and every 5 minutes
 * for the last day. A single unified walk (rather than four independently-generated
 * series) so every timeframe reads a consistent history instead of disconnected shapes.
 */
function buildPriceHistory(seedKey: string, endPrice: number): PricePoint[] {
  const boundaries = [NOW - 365 * DAY, NOW - 7 * DAY, NOW - DAY, NOW];
  const stepsByLeg = [DAY, HOUR, 5 * MINUTE];

  const timestamps: number[] = [];
  for (let leg = 0; leg < stepsByLeg.length; leg++) {
    for (let t = boundaries[leg]; t < boundaries[leg + 1]; t += stepsByLeg[leg]) {
      timestamps.push(t);
    }
  }
  timestamps.push(NOW);
  ...
  const volatilityPerDay = 0.9;
  const walk: number[] = [0];
  for (let i = 1; i < timestamps.length; i++) {
    const dtDays = (timestamps[i] - timestamps[i - 1]) / DAY;
    // Brownian-style scaling so a coarse (daily) step isn't as jumpy as a fine (5-min) one.
    const shock = (rand() - 0.5) * 2 * volatilityPerDay * Math.sqrt(Math.max(dtDays, 1 / (24 * 12)));
  ...
  return timestamps.map((t, i) => ({
    t,
    // Normalized so the walk ends exactly at endPrice, preserving relative shape.
    price: Math.max(endPrice * (1 + (walk[i] - lastWalk) * 0.15), endPrice * 0.05),
  }));
}
