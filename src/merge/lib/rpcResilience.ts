...
/** Bounded exponential backoff with jitter, retrying ONLY genuine rate-limit errors -- any other error is rethrown immediately, never masked by a pointless retry loop. Safe to use on any READ-ONLY call; never wrap a transaction-submitting call with this (see confirmSignatureBounded below for why submission itself is never auto-retried). */
