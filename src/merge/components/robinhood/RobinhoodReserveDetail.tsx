    ...
    <div className="container mx-auto px-4 md:px-8 py-10 space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 pb-4 border-b border-border/60">
        <div>
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-3xl font-merge-display font-bold">{snap.name}</h1>
            <Badge variant="secondary">{snap.symbol}</Badge>
            <span className="badge-verified">Live on Robinhood Chain</span>
          </div>
          <a href={explorerAddr(address)} target="_blank" rel="noopener noreferrer" className="font-merge-mono text-xs text-muted-foreground hover:text-primary">
            CA: {address}
          </a>
        </div>
        <Button onClick={connect}>{account ? short(account) : "Connect EVM wallet"}</Button>
      </div>
      {connectErr && <p className="text-sm text-destructive">{connectErr}</p>}

      <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
        {[
          ["Price", usd(snap.navPerShare, 4)],
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Values marked to USDG at Uniswap v3 spot.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Deposit the basket in kind; receive shares.</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Burn shares; receive the basket in kind.</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="text-sm">
            ...
            <Row label="DAO share of it" value={`${fmtUnits(snap.daoFeeBps, 2, 2)}%`} />
            <Row label="DAO floor" value={pctFromD18(snap.feeFloor)} />
            <Row label="Auction cap" value={`${snap.maxAuctionLength}s`} />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="text-sm">
            {!account ? (
              <p className="text-muted-foreground">Connect an EVM wallet to see your shares and balances.</p>
            ) : (
              <>
                <Row label="Account" value={<span className="text-xs">{short(account)}</span>} />
                <Row
                  label="Gas (ETH)"
                  value={<span className={gas === 0n ? "text-destructive" : ""}>{gas === null ? "—" : fmtUnits(gas, 18, 6)}{gas === 0n ? " — you need gas to send" : ""}</span>}
                />
                <Row label="Your shares" value={fmtUnits(myShares, dec, 6)} />
                {myAssets.map((a) => <Row key={a.symbol} label={`Your ${a.symbol}`} value={a.text} />)}
              </>
            )}
          </CardContent>
        </Card>
