    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="mb-8">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <h1 className="text-3xl font-display font-bold tracking-tight mb-1">Manager Dashboard</h1>
            <div className="flex items-center gap-2">
              <span className="font-semibold">{dtr.name}</span>
              <Badge variant="secondary" className="font-mono text-xs">{dtr.ticker}</Badge>
        ...
        <div className="md:col-span-3">
          {activeTab === "overview" && (
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-xl font-display">Reserve Identity</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-mono font-medium">{dtr.ticker}</p>
                    </div>
                  </div>
                  <div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-xl font-display">Performance & Scale</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="p-4 bg-muted/30 rounded-lg border border-border/50">
                      <p className="text-xs text-muted-foreground mb-1">AUM</p>
                      <p className="font-mono font-bold text-lg">{formatUsdc(dtr.aum, { compact: true })}</p>
                    </div>
                    <div className="p-4 bg-muted/30 rounded-lg border border-border/50">
                      <p className="text-xs text-muted-foreground mb-1">NAV per Token</p>
                      <p className="font-mono font-bold text-lg">{formatUsdc(dtr.nav)}</p>
                    </div>
                    <div className="p-4 bg-muted/30 rounded-lg border border-border/50">
                      <p className="text-xs text-muted-foreground mb-1">Holders</p>
                      <p className="font-mono font-bold text-lg">{dtr.holders.toLocaleString()}</p>
                    </div>
                    <div className="p-4 bg-muted/30 rounded-lg border border-border/50">
                      <p className="text-xs text-muted-foreground mb-1">Total Delegates</p>
                      <p className="font-mono font-bold text-lg">{dtr.delegates.length}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    <CardTitle className="text-xl font-display">Active Delegates</CardTitle>
                      ...
                      {dtr.delegates.map(del => (
                        <div key={del.address} className="border border-border rounded-lg p-4 bg-card/50">
                          {editingDelegate === del.address ? (
                            <div className="space-y-4">
                              <p className="font-mono text-sm break-all mb-4">{del.address}</p>
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                              <div className="space-y-2">
                                <p className="font-mono text-sm break-all">{del.address}</p>
              ...
              {hasManageDelegates && (
                <Card>
                  <CardHeader>
                    <CardTitle className="text-lg font-display">Add New Delegate</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
          ...
          {activeTab === "rebalance" && (
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-xl font-display">Portfolio Rebalance</CardTitle>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {dtr.composition.map(asset => {
                          ...
                          const newWeight = isEdited ? rebalanceEdits[asset.symbol] : asset.weight;
                          ...
                          return (
                            <TableRow key={asset.symbol} className={isEdited ? "bg-muted/50" : ""}>
                              <TableCell className="font-medium">
                                {asset.name} <span className="text-muted-foreground text-xs font-mono ml-1">{asset.symbol}</span>
                              </TableCell>
                              ...
                              <TableCell className="text-right">
                                <div className="flex items-center justify-end">
                                  <Input 
                                    type="number"
                                    className={`w-20 h-8 text-right font-mono ${isEdited ? 'border-primary' : ''}`}
                  ...
                  <div className="p-4 border-t border-border mt-4 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div className="space-y-1 text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`font-mono font-bold ${previewTotal > 1.0001 ? 'text-destructive' : 'text-primary'}`}>
                      ...
                      {preview.unallocated > 0 && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-mono font-medium">{(preview.unallocated * 100).toFixed(1)}%</span>
                        </div>
                      )}
                      <div className="flex items-center gap-2 pt-1 border-t border-border/50">
                        <span className="text-muted-foreground text-xs">Total (assets + unallocated):</span>
                        <span className="font-mono text-xs font-medium">{(previewTotal * 100).toFixed(1)}%</span>
                      </div>
                    </div>
